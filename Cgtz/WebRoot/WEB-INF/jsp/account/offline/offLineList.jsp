			...
			<table id="table" class="datatable" summary="list of members in EE Studay">
				...
				<c:forEach items="${ pm.data}" var="data" varStatus="status">
					<tr <c:if test="${data.isEnough eq '0' }">style="color:red"</c:if> >
						...
						<td>${data.appState}</td>
						<td>
							<a href="javascript:void(0);" onclick="javascript:updateData('${ data.id}','${data.contractId }','${data.name }','${data.productName }');">放款</a>&nbsp;
						</td>
					</tr>
				</c:forEach>
			</table>
...
function updateData(data_id,cont_id,name,product_type) {
	var url = "<%=basePath%>" + "account/offline/update.do?id=" + data_id + "&contractId=" + cont_id;
