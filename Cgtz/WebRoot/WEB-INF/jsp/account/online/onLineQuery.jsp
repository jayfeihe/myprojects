...
<body>
<div class="easyui-tabs" id="onLineQueryTabs" data-options="fit:true">
	<div title="线上放款列表">
		<div id="main">
			<div id="part1" class="part">
				...
				<div class="content">
					<form id="queryForm" action="account/online/list.do" method="post" target="queryContent">
						<table>
							<tr>
								<td>线下合同编号:</td>
								<td><input id="contractId" name="contractId" type="text" class="textbox"/></td>
								<td>线上合同编号:</td>
								<td><input id="onLineContractId" name="onLineContractId" type="text" class="textbox"/></td>
...
function addTab(title, url){
    if ($('#onLineQueryTabs').tabs('exists', title)){
        $('#onLineQueryTabs').tabs('select', title);
    } else {
        var content = '<iframe scrolling="auto" frameborder="0"  src="'+url+'" style="width:100%;height:100%;"></iframe>';
        $('#onLineQueryTabs').tabs('add',{
            title:title,
            content:content,
            closable:true
        });
...
//提交后，删除当前选项卡
function removeTab(){
	var tab = $('#onLineQueryTabs').tabs('getSelected');
	var tabIndex=$('#onLineQueryTabs').tabs('getTabIndex',tab);
	$('#onLineQueryTabs').tabs('close',tabIndex);
