...
<body>
	<div class="easyui-tabs" id="onlineUpdateTabs" style="width: 100%;" data-options="fit:true">
		<div title="线上放款" style="width: 100%;padding:2px">
			<div style="margin-left: 10px;margin-top: 10px; font-size: 14px;"><strong>合同信息</strong></div><hr color="#D3D3D3"/>
			<!-- 直投 -->
			<c:if test="${contract.getLoanWay eq '1' }">
				<table>
					<tr>
						<td>开户名:</td>
						<td>
							<input id="acctName" type="text" class="textbox easyui-validatebox" readonly="readonly"
								data-options="validType:['length[0,20]']" value="${loanBase.acctName}"/>
						</td>
						<td>账号:</td>
						<td><input id="acctCode" type="text" class="textbox" style="width:150px;" readonly="readonly"
					...
					<tr>
						<td>放款金额:</td>
						<td>
							<input type="text" class="textbox" value="${onLineAmt}" readonly="readonly"/>元
						</td>
						<td>融资方式:</td>
			...
			<!-- 债权转让 -->
			<c:if test="${contract.getLoanWay eq '2'}">	
				...
				<table>
					<tr>
						...
						<td>
							<input id="acctName" type="text" class="textbox easyui-validatebox" readonly="readonly"
								data-options="validType:['length[0,20]']" value="${lendUser.acctName}"/>
						</td>
						<td>账号:</td>
						<td><input id="acctCode" type="text" class="textbox easyui-validatebox" style="width:150px;" readonly="readonly"
								value="${lendUser.acctCode}"/>
						</td>
					</tr>
					<tr>
						<td>开户行:</td>
						<td colspan="6">
							<input id="acctPrvn" type="text" class="textbox easyui-combobox" readonly="readonly"
								data-options="editable:false" value="${lendUser.acctPrvn}"/>省
							<input id="acctCity" type="text" class="textbox easyui-combobox" readonly="readonly"
								data-options="editable:false" value="${lendUser.acctCity}"/>市
							<input id="acctCtry" type="text" class="textbox easyui-combobox" readonly="readonly"
								data-options="editable:false" value="${lendUser.acctCtry}"/>区/县
							<input id="acctBank" type="text" class="textbox easyui-combobox" readonly="readonly"
								data-options="editable:false" value="${lendUser.acctBank}"/>开户行
							<input id="acctBranch" type="text" class="textbox easyui-combobox" readonly="readonly"
								data-options="editable:true" value="${lendUser.acctBranch}" style="width: 250px;"/>支行
						</td>
					</tr>
					<tr>
						<td>放款金额:</td>
			...
			<form id="onlineUpdateForm">
				...
				<input type="hidden" name="onLineContractId" value="${onLineContractId}"/>
				<input type="hidden" name="amt" value="${onLineAmt}"/>
...
function updateFunction() {
	//验证表单验证是否通过
	if(false == $('#onlineUpdateForm').form('validate')){
		$.messager.confirm('消息', "页面有必输字段，但未填值！");
		return;
	}
	
	//去掉 input 输入的 前后空格
	$('#onlineUpdateForm').find('input').each(function(){
	...
	var params = $('#onlineUpdateForm').serialize();
	
	//按钮失效防点击
	$(".btn").attr("disabled", true);
	$.ajax({
		type : "POST",
		url : "<%=basePath%>" + "account/online/save.do",
		data : params,
