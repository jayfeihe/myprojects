...
<div title="贷后综合查询">
	...
	<div id="part1" class="part">
		...
		<div class="content">
			<form id="queryAfterLoanForm" action="afterLoanQ/list.do" method="post" target="queryContent">
				<table>
<tr>
<td>合同状态:</td>
...
<tr> 
<td>是否逾期:</td>
<td><input id="isLate" name="isLate" type="text" class="textbox easyui-combobox" 
data-options="editable:false,panelHeight:'auto',valueField:'keyProp',textField:'keyValue',
...
data-options="editable:false,panelHeight:'auto',url:'<%=basePath%>sys/org/selectList.do',method:'get'"/></td>
...
-<input id="maxEndTime" name="maxEndTime" type="text" editable="false" class="textbox easyui-datebox" style="width:100px;"/></td>
					...
					<tr><td></td>
						<td>
							<input type="button" value="查询" class="btn" onclick="submitForm('queryAfterLoanForm')"/>
							<input type="button" value="重置" class="btn" onclick="$('#queryAfterLoanForm').form('clear');"/>
						</td>
						<td></td>
					</tr>
				</table>	
...
function removeTab(){
	...
	submitForm("queryAfterLoanForm");//解决Tab提交关闭列表页刷新的问题
}
...
function submitForm(fromId) {
	//去掉 input 输入的 前后空格
	$('#queryAfterLoanForm').find('input').each(function(){
