	...
	<div title="分公司审批列表">
		<div id="main">
			<div id="part1" class="part">
				...
				<div class="content">
					<form id="queryForm" action="branch/list.do" method="post" target="queryContent">
						<table>
							<tr>
								<td>申请编号:</td>
								...
								<td><input id="salesmanName" name="salesmanName" type="text" class="textbox" />
									<%-- <input id="salesman" name="salesman" type="text"
										data-options="editable:false,
													  url:'sys/user/listUserByOrgAndRole.do?orgId=${login_org.orgId}&roleNames=<%=CommonConstant.ROLE_SALESMAN %>',
													  textField:'name',
													  valueField:'loginId',
													  panelHeight:'auto',
													loadFilter:function(data){
...
function addTab(title, url){
    if ($('#appQueryTabs').tabs('exists', title)){
        $('#appQueryTabs').tabs('select', title);
    } else {
        var content = '<iframe scrolling="auto" frameborder="0"  src="'+url+'" style="width:100%;height:100%;"></iframe>';
        $('#appQueryTabs').tabs('add',{
            title:title,
            content:content,
            closable:true
        });
...
//提交后，删除当前选项卡
function removeTab(){
	var tab = $('#appQueryTabs').tabs('getSelected');
	var tabIndex=$('#appQueryTabs').tabs('getTabIndex',tab);
	$('#appQueryTabs').tabs('close',tabIndex);
