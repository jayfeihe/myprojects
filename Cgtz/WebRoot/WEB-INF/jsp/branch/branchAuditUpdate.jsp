...
<body>
	<div class="easyui-tabs" id="branchUpdateTabs" style="width: 100%;" data-options="fit:true">
...
function updateFunction() {
	//去掉 input 输入的 前后空格
	$('#branchUpdateForm').find('input').each(function(){
	...
	//验证表单验证是否通过
	if(false == $('#branchUpdateForm').form('validate') ){
	...
	var params = $('#branchUpdateForm').serialize();
	
	//按钮失效防点击
	$(".btn").attr("disabled", true);
	$.ajax({
		type : "POST",
		url : "<%=basePath%>" + "branch/save.do",
		data : params,
...
$(document).ready(function (){
	$("#decision").combobox({
		...
		onChange:function(nVal,oVal){
			if("0" == nVal) {
				...
				var url = "common/nodeorder/listNode.do?name=" + "<%=CommonConstant.PROCESS_B%>" + "&type=1";
				$('#node').combobox('clear');
	...
	$("#node").combobox({
		url:'common/nodeorder/listNode.do?name=<%=CommonConstant.PROCESS_B%>&type=1',
		textField:'keyValue',
		valueField:'keyProp'
	});
}); 
