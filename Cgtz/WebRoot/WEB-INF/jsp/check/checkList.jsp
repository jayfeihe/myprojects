...
<c:choose>
<c:when test="${data.overdueType eq '1' }">${data.dueInterest}</c:when>
<c:when test="${data.overdueType eq '2' }">${data.dueCapital}</c:when>
</c:choose>
</fmt:formatNumber>元
</td>
<td>${data.lateNum}</td>
...
//逾期申请受理
function overdueAccept(data_id,contractId,num) {
	var title="【合同号-"+contractId+"】逾期申请受理";
	addTab(title,'<%=basePath%>' + 'check/overdueAccept/update.do?loanId=' + data_id+'&contractId='+contractId+'&lateNum='+num);
	return;
}
//页面加载完动作
