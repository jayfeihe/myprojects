...
<td><a href="javascript:void(0)" onclick="clltOpera('${ data.loanId}','${data.contractId}')" style="text-decoration: underline;">催收</a>&nbsp;</td>
					</tr>
				</c:forEach>
			</table>
...
//查看申请
function readLoanData(id){
    addTab('申请信息','<%=basePath%>'+'asset/manage/allRead.do?loanId='+id);
}
function clltOpera(loanId,contractId){
   title="【合同号-"+contractId+"】催收日志";
   var url='<%=basePath%>'+'cllt/clltlog/query.do?loanId='+loanId+'&contractId='+contractId;
    addTab(title,url);
}
//全选、取消全选的事件  
function selectAll(){  
