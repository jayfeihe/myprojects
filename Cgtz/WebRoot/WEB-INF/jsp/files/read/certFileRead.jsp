 ...
 <c:if test="${not empty imgCount and imgCount gt 0 }">
	<div id="toolbar" style="font-size: 14px;" ><strong>图片资料</strong><hr width="100%" size="2" color="#D3D3D3" align="center" noshade/></div>
	 <table width="100%">
		 <tr>
		 	<td colspan="3">
		 		<a class="cert_category_a" href="javascript:void(0)" onclick="javascript:showCertImage('${loanId }','${bizKey}','${sec }','');">全部[${imgCount }张]</a> &nbsp; &nbsp;
		 		<c:forEach items="${imgCategories}" var="img" varStatus="status" >
					<a class="cert_category_a" href="javascript:showCertImage('${loanId }','${bizKey}','${sec }','${img.category}');">
		...
		<tr>
			<td align="center">
				<div id= "showCertImg" style="width: 80%;margin-top: 10px;">
	...
	<script type="text/javascript">
		function loadCertImage(loanId,bizKey,sec,category){
		...
		function showCertImage(loanId,bizKey,sec,category){
			var imgurl="<%=basePath%>files/getImgs.do?timestamp=" + (new Date()).getTime();
			if(loanId!=null&& loanId!="") {
				imgurl=imgurl+"&loanId="+loanId;
				if(bizKey!=null&& bizKey!=""){
		...
		loadCertImage("${loanId}","${bizKey}","${sec}","");
	
		$(".cert_category_a").click(function(){
			$(".cert_category_a").css({"color":"#000","font-weight":"normal"});
