 ...
 <c:if test="${not empty imgCount and imgCount gt 0 }">
	<div id="toolbar" style="font-size: 14px;" ><strong>图片资料</strong><hr width="100%" size="2" color="#D3D3D3" align="center" noshade/></div>
	 <table width="100%">
		 <tr>
		 	<td colspan="3">
		 		<a class="contract_category_a" href="javascript:void(0)" onclick="javascript:showContractImage('${loanId }','${bizKey}','${sec }','');">全部[${imgCount }张]</a> &nbsp; &nbsp;
		 		<c:forEach items="${imgCategories}" var="img" varStatus="status" >
					<a class="contract_category_a" href="javascript:showContractImage('${loanId }','${bizKey}','${sec }','${img.category}');">
		...
		<tr>
			<td align="center">
				<div id= "showContractImg" style="width: 80%;margin-top: 10px;">
	...
	<script type="text/javascript">
		function loadContractImage(loanId,bizKey,sec,category){
				...
				$.ajax({
					...
					success : function(data) {
						if(data.length>0) {
	                		...
	                		$("#showContractImg").html(html.join(""));
						}
					},
					error : function() {
		...
		function showContractImage(loanId,bizKey,sec,category){
			var imgurl="<%=basePath%>files/getImgs.do?timestamp=" + (new Date()).getTime();
			if(loanId!=null&& loanId!="") {
				imgurl=imgurl+"&loanId="+loanId;
				if(bizKey!=null&& bizKey!=""){
		...
		loadContractImage("${loanId}","${bizKey}","${sec}","");
	
		$(".contract_category_a").click(function(){
			$(".contract_category_a").css({"color":"#000","font-weight":"normal"});
