	...
	<div class="easyui-tabs" id="riskFirstUpdateTabs" style="width: 100%;" data-options="fit:true">
		<div title="审核" style="width: 100%;padding:2px">
			<form id="riskUpdateForm">
				...
				<div style="margin-left: 10px;margin-top: 10px; font-size: 14px;"><strong>评审会意见</strong></div><hr color="#D3D3D3"/>
				<table id="table" class="datatable" summary="list of members in EE Studay">
					...
					<c:forEach items="${judgeAdvs}" var="data" varStatus="status">
						<tr>
							...
							<td>
								<c:choose>
									<c:when test="${data.auditState eq '1'}">未处理</c:when>
									<c:when test="${data.auditState eq '2'}">已处理</c:when>
								</c:choose>
							</td>
							<td>${data.auditAdv}</td>
						</tr>
...
function updateFunction() {
	...
	$.ajax({
		type : "POST",
		url : "<%=basePath%>" + "judge/review/save.do",
		data : params,
...
$(document).ready(function (){
	...
	$("#decision").combobox({
		...
		onChange:function(nVal,oVal){
			if("0" == nVal) {
				...
				var url = "common/nodeorder/listNode.do?name=" + "<%=CommonConstant.PROCESS_E%>" + "&type=2";
				$('#node').combobox('clear');
	...
	$("#node").combobox({
		url:'common/nodeorder/listNode.do?name=<%=CommonConstant.PROCESS_E%>&type=2',
		textField:'keyValue',
		valueField:'keyProp'
	});
}); 
