...
<body>
	<div class="easyui-tabs" id="lawAdviceUpdateTabs" style="width: 100%;" data-options="fit:true">
		<div title="法律意见" style="width: 100%;padding:2px">
			<form id="lawAdviceUpdateForm">
				<input type="hidden" name="loanId" value="${loanBase.loanId}"/>
				<div style="margin-left: 10px;margin-top: 10px; font-size: 14px;"><strong>意见</strong></div><hr color="#D3D3D3"/>
				<table>
					<tr>
						<td>说明:</td>
						<td colspan="6">
			...
			<jsp:include page="/files/load.do">
				...
				<jsp:param value="filesce13" name="sec"/>
		...
		<div title="打款凭证" data-options="href:'${basePath}files/read2.do?loanId=${loanBase.loanId}&sec=filesce12&bizKey=${contract.contractId}'" style="padding:10px"></div>
...
function updateFunction() {
	//验证表单验证是否通过
	if(false == $('#lawAdviceUpdateForm').form('validate')){
		$.messager.confirm('消息', "页面有必输字段，但未填值！");
		return;
	}
	
	//去掉 input 输入的 前后空格
	$('#lawAdviceUpdateForm').find('input').each(function(){
	...
	var params = $('#lawAdviceUpdateForm').serialize();
	
	//按钮失效防点击
	$(".btn").attr("disabled", true);
	$.ajax({
		type : "POST",
		url : "<%=basePath%>" + "law/advice/save.do",
		data : params,
