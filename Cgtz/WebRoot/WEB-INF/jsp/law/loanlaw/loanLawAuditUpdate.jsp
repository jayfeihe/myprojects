			...
			<form id="updateLawForm" >
				...
				<table id="lawInfo">
					...
					<tr>
						<td>诉讼情况:</td>
						<td>
							<input id="lawState" name="lawState" type="text" class="textbox easyui-combobox" readonly="readonly"
				...
				<div style="margin-left: 10px;margin-top: 10px; font-size: 14px;"><strong>诉讼复核</strong></div><hr color="#D3D3D3"/>
				<table>
					<tr>
						<td>诉讼复核:</td>
						<td>
							<input type="text" name="lawCheckState" class="textbox easyui-combobox" 
								data-options="required:true,editable:false,panelHeight:'auto',
											textField:'keyValue',
											valueField:'keyProp',
											data:dataDictJson.decision" value="${bean.lawCheckState }"/>
						</td>
					</tr>
					<tr>
						<td>说明:</td>
						<td colspan="6">
								<textarea name="lawCheckRemark" class="textbox easyui-validatebox"
									data-options="validType:['length[0,500]']" 
									style="resize: none;width:625px;height:50px!important;">${bean.lawCheckRemark }</textarea>
						</td>
					</tr>
					<tr>
						<td colspan="3">
...
function updateFunction(type) {
	...
	$.ajax({
		type : "POST",
		url : "<%=basePath%>" + "law/loanlaw/save.do",
		...
		success : function(data) {
			...
			if ("true"==data.success) {
				$.messager.alert('消息', "成功", 'info', function(){
					window.parent.refreshTab("lawFirstUpdateTabs");
            	});
            } else {				
