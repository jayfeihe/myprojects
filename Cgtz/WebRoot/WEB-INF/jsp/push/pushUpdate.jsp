...
<body>
	<div class="easyui-tabs" id="offlineUpdateTabs" style="width: 100%;" data-options="fit:true">
		<div title="推送线上" style="width: 100%;padding:2px">
			<form id="pushUpdateForm">
				...
				<div style="margin-left: 10px;margin-top: 10px; font-size: 14px;"><strong>推送说明</strong></div><hr color="#D3D3D3"/>
				<table>
					<tr>
						<td>说明:</td>
						<td colspan="6">
					...
					<tr>
						<td colspan="3">
							<input type="button" value="推送" class="btn" onclick="updateFunction()"/>&nbsp;&nbsp;
						</td>
					</tr>
				</table>
			</form>
			...
			<jsp:include page="/files/load.do">
				<jsp:param value="${origLoanBase.loanId }" name="origLoanId"/>
				<jsp:param value="${loanBase.loanId }" name="loId"/>
				<jsp:param value="filesce10" name="sec"/>
...
function updateFunction() {
	//验证表单验证是否通过
	if(false == $('#pushUpdateForm').form('validate')){
		$.messager.confirm('消息', "页面有必输字段，但未填值！");
		return;
	}
	
	//去掉 input 输入的 前后空格
	$('#pushUpdateForm').find('input').each(function(){
	...
	var params = $('#pushUpdateForm').serialize();
	
	//按钮失效防点击
	$(".btn").attr("disabled", true);
	$.ajax({
		type : "POST",
		url : "<%=basePath%>" + "push/save.do",
		data : params,
