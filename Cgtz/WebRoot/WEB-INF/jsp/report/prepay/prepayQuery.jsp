...
<body>
<div class="easyui-tabs" id="prepayTabs" data-options="fit:true">
<div title="线下线上垫付表">
	<div id="main">
	<div id="part1" class="part">
		...
		<div class="content">
		<form id="queryPrepayForm" action="report/prepay/list.do" method="post" target="queryContent">
 <input type="hidden" id="subject" name="subject" value="付线上垫付"/>
<table>
<tr>
<td>时间:</td>
...
<td><input id="proof" name="proof" type="text"  class="textbox easyui-validatebox"/>
					...
					<tr><td></td>
						<td>
							<input type="button" value="查询" class="btn" onclick="submitForm('queryPrepayForm')"/>
							<input type="button" value="导出" class="btn" onclick="exportExcel('queryPrepayForm')"/>
							<input type="button" value="重置" class="btn" onclick="$('#queryPrepayForm').form('clear');"/>
						</td>
						<td></td>
					</tr>
				</table>	
...
function submitForm(fromId) {
	//去掉 input 输入的 前后空格
	$('#queryPrepayForm').find('input').each(function(){
...
function exportExcel(fromId) {
	$('#' + fromId).attr("action","<%=basePath%>report/prepay/excel.do");
