...
<body>
<div class="easyui-tabs" id="reportOverdueTabs" data-options="fit:true">
<div title="分公司逾期统计表">
	<div id="main">
	<div id="part1" class="part">
		...
		<div class="content">
			<form id="queryReportOverdueForm" action="report/reportOverdue/list.do" method="post" target="queryContent">
				<table>
<tr>
<%-- <td>所属机构:</td>
...
<td>日期:</td>
<td><input id="createDate" name="createDate" class="easyui-datebox" required="true" editable="false" style="width:90px;"/></td>
<td></td>
						<td>
							<input type="button" value="查询" class="btn" onclick="submitForm('queryReportOverdueForm')"/>
							<input type="button" value="导出" class="btn" onclick="exportExcel('queryReportOverdueForm')"/>
							<input type="button" value="重置" class="btn" onclick="$('#queryReportOverdueForm').form('clear');"/>
						</td>
						<td></td>
					</tr>
				</table>	
...
function submitForm(fromId) {
	//验证，必须输入日期
	if(false == $('#queryReportOverdueForm').form('validate') ){
		$.messager.confirm('消息', "请选择日期！");
		return;
	}
	//去掉 input 输入的 前后空格
	$('#queryReportOverdueForm').find('input').each(function(){
...
function exportExcel(fromId) {
	$('#' + fromId).attr("action","<%=basePath%>report/reportOverdue/excel.do");
...
/* 
	submitForm("queryReportOverdueForm"); */
});
</script>
