...
<body>
	<div class="easyui-tabs" id="riskFirstUpdateTabs" style="width: 100%;" data-options="fit:true">
		...
		<div title="核价" data-options="href:'${basePath}risk/price/query.do?loanId=${loanBase.loanId}'" style="width: 100%;padding:2px"></div>
...
function updateFunction() {
	/* if($(".tab_show_price").length == 0){
		$.messager.alert('消息', "请进行核价操作");
		return;
	} */
	...
	$.ajax({
		type : "POST",
		url : "<%=basePath%>" + "risk/first/save.do",
		data : params,
...
$(document).ready(function (){
	$("#decision").combobox({
		...
		onChange:function(nVal,oVal){
			if("0" == nVal) {
				...
				var url = "common/nodeorder/listNode.do?name=" + "<%=CommonConstant.PROCESS_C%>" + "&type=1";
				$('#node').combobox('clear');
	...
	$("#node").combobox({
		url:'common/nodeorder/listNode.do?name=<%=CommonConstant.PROCESS_C%>&type=1',
		textField:'keyValue',
		valueField:'keyProp'
	});
}); 
