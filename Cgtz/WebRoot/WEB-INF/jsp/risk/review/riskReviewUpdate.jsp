...
function updateFunction() {
	...
	$.ajax({
		type : "POST",
		url : "<%=basePath%>" + "risk/review/save.do",
		data : params,
...
$(document).ready(function (){
	...
	$("#decision").combobox({
		...
		onChange:function(nVal,oVal){
			if("0" == nVal) {
				...
				var url = "common/nodeorder/listNode.do?name=" + "<%=CommonConstant.PROCESS_F%>" + "&type="+type;
				$('#node').combobox('clear');
	...
	$("#node").combobox({
		url:'common/nodeorder/listNode.do?name=<%=CommonConstant.PROCESS_F%>&type='+type,
		textField:'keyValue',
		valueField:'keyProp'
	});
}); 
