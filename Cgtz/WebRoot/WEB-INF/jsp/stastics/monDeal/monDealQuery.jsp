...
<div title="分公司每月成交量历史统计">
	<div id="main">
	<div id="part1" class="part">
		<p class="title"><a href="javascript:void(0);">查询</a></p>
		
		<div class="content">
			<form id="queryMonDealForm" action="stastics/monDeal/list.do" method="post" target="queryMonDealContent">
				<table>
<tr>
<%-- <td>分公司:</td>
...
<td></td>
						<td>
							<input type="button" value="查询" class="btn" onclick="submitMonDealForm('queryMonDealForm')"/>
							<input type="button" value="导出" class="btn" onclick="exportExcel('queryMonDealForm')"/>
							<input type="button" value="重置" class="btn" onclick="$('#queryMonDealForm').form('clear');"/>
						</td>
						<td></td>
					</tr>
				</table>	
...
<script type="text/javascript">
function submitMonDealForm(fromId) {
	//去掉 input 输入的 前后空格
	$('#queryMonDealForm').find('input').each(function(){
...
function exportExcel(fromId) {
	$('#' + fromId).attr("action","<%=basePath%>stastics/monDeal/excel.do");
...
//填充select数据样例
	...
	/* submitMonDealForm("queryMonDealForm"); */
});
</script>
