...
<div title="分公司存量历史统计">
	...
	<div id="part1" class="part">
		...
		<div class="content">
			<form id="querySaveDealsForm" action="stastics/saveDeals/list.do" method="post" target="querySaveDealsContent">
				<table>
<tr>
<%-- <td>分公司:</td>
...
<td>统计时间:</td>
<td><input id="inputDate" name="inputDate" type="text" class="textbox easyui-datebox" required="true" editable="false"/></td>
						<td>
							<input type="button" value="查询" class="btn" onclick="submitSaveDealsForm('querySaveDealsForm')"/>
							<input type="button" value="导出" class="btn" onclick="exportExcel('querySaveDealsForm')"/>
							<input type="button" value="重置" class="btn" onclick="$('#querySaveDealsForm').form('clear');"/>
						</td>
						<td></td>
					</tr>
				</table>	
...
<script type="text/javascript">
function submitSaveDealsForm(fromId) {
	//去掉 input 输入的 前后空格
	$('#querySaveDealsForm').find('input').each(function(){
		if($(this).attr("type")!="file"){
			$(this).val($.trim($(this).val()));
		}
	});
	//验证表单验证是否通过
	if(false == $('#querySaveDealsForm').form('validate') ){
...
function exportExcel(fromId) {
	$('#' + fromId).attr("action","<%=basePath%>stastics/saveDeals/excel.do");
...
//填充select数据样例
	...
	/* submitSaveDealsForm("querySaveDealsForm"); */
});
</script>
