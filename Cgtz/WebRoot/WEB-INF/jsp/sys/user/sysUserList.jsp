...
<%--				<a href="javascript:void(0);" onclick="javascript:showQueryContent('but_updown_id');"><img id="but_updown_id" src="img/square/but_up.png" class='dotimg' title="打开/关闭查询模板" /></a>&nbsp;--%>
				<a href="javascript:void(0);" onclick="addTab('添加用户','<%=basePath%>sys/user/add.do')"><img src="img/square/sys_but_add.png" class='dotimg' title="添加用户" /></a>&nbsp;
<%--				<a href="javascript:void(0);" onclick="javascript:sysUserExcel();"><img src="img/square/but_excel.png" class='dotimg' title="导出EXCEL文件" /></a>&nbsp;--%>
			...
			<table id="table" class="datatable" summary="list of members in EE Studay">
				...
				<td>
					<c:choose>
						<c:when test="${data.state eq 0}">关闭</c:when>
						<c:when test="${data.state eq 1}">启用</c:when>
						<c:when test="${data.state eq 2}">挂起</c:when>
					</c:choose>
				</td>
				<td>
					<a href="javascript:void(0);" onclick="addTab('【用户ID-${data.id}】修改用户','<%=basePath%>sys/user/update.do?id=${data.id}')" title="修改">修改用户</a>&nbsp;
					<a href="javascript:void(0);" onclick="addTab('【用户ID-${data.id}】修改密码','<%=basePath%>sys/user/update.do?id=${data.id}&opt=pwd')" title="修改密码">修改密码</a>&nbsp;
