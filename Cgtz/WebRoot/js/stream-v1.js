...
(function(){
	...
	var StreamUploader = function(){
		...
		this.config = {
			...
			uploadURL : "",
			serverAddress : "",
			portionSize : 10485760,   //分片大小  默认10m
			parameters : {},
			fileFieldName : "FileData",
	...
	StreamUploader.prototype = {
		...
		initializer: function(file){
			...
			this.file = null;
			this.fileId = null;
			this.filePiece = 10485760;/** 分片大小  默认10m */
	...
	function Main(cfg){
		...
		this.config = {
			...
			dragAndDropTips: cfg.dragAndDropTips || "<span>把文件拖拽到这里</span>",
			...
			tokenURL : cfg.tokenURL || "files/tk.do",
			frmUploadURL : cfg.frmUploadURL || Browser.firefox ? "files/fd.do;" + document.cookie : "files/fd.do",
			uploadURL : cfg.uploadURL || "files/upload.do"
		};
		Parent.apply(this, arguments);
	}
	...
	Main.prototype = {
		...
		onSelect : function(list) {
			...
			fShowMessage("选中文件数量: " + list.length);
		},
		onFileCountExceed : function(selected, limit) {
			fShowMessage("File counts:" + selected + ", but limited:" + limit, true);
			$.messager.alert('提示',"选中文件数量: "  + selected + ", 系统限制为:" + limit);
		},
		onMaxSizeExceed : function(file) {
			fShowMessage("文件:" + file.name + " 大小为:" + file.size +" 上传限制大小为:" + file.limitSize, true);
			$.messager.alert('提示',"文件:" + file.name + " 大小为:" + file.size +" 上传限制大小为:" + file.limitSize);
		},
		onExtNameMismatch: function(file) {
			fShowMessage("允许的扩展名为: [" + file.filters.toString() + "], 不能提交 " + file.name, true);
			$.messager.alert('提示',"允许的扩展名为: [" + file.filters.toString() + "], 不能提交 " + file.name);
		},
		onAddTask: function(file) {
			fShowMessage("Add to task << name: [" + file.name);
		},
		onCancel : function(file) {
			fShowMessage("取消上传: " + file.name);
		},
		onStop : function() {
			fShowMessage("停止上传!");
		},
		onCancelAll : function(numbers) {
			fShowMessage(numbers + " 文件取消! ");
		},
		onComplete : function(file) {
			fShowMessage("文件:" + file.name + ", 大小:" + file.size + " 上传完成[OK]");
		},
		onQueueComplete : function(msg) {
			fShowMessage("上传队列完成(msg: "+msg+")	---==>		[OK]");
		},
		onUploadError : function(status, msg) {
			fShowMessage("Error Occur.  Status:" + status + ", Message: " + msg, true);
			$.messager.alert('错误',"发生错误！状态： " + status+ ", 消息: " + msg);
		},
		disable : function() {
		...
		upload : function(index) {
			//是否选中
			catId=$('#catId').combobox('getValue');
			if(catId== '' || catId == null){
				//alert("请选择文件类型");
				$.messager.alert('提示','请选择文件类别!');
				return;
			}
			var bizKey = $("#biz").val();
			if(bizKey == '' || bizKey == null){
				$.messager.alert('提示','请先进行保存!');
				return;
			}
			...
			var frmUploadURL = this.get("frmUploadURL")+"?loanId="+$("#loanId").val();
			var uploadURL = this.get("uploadURL")+"?loanId="+$("#loanId").val()+"&biz="+$("#biz").val()+ "&secId="+$("#secId").val()+"&catId="+ $("#catId").combobox("getText");
