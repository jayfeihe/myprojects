...
import com.tera.audit.account.service.IFinanceReviewService;
 ...
 */
@Controller
@RequestMapping("/account/financeReview")
public class FinanceReviewController extends BaseController {

	private final static Logger log = Logger.getLogger(FinanceReviewController.class);
	
	@Autowired(required=false) //自动注入
	private ILoanBaseService loanBaseService;
	@Autowired
	private IFinanceReviewService financeReviewService;
	@Autowired
	...
	public String financeReviewQuery(HttpServletRequest request, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		log.info(thisMethodName+":end");
		return "account/finance/financeReviewQuery";
	}
	...
	public String financeReviewList(HttpServletRequest request, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		...
		queryMap.put("bpmStates", new String[]{CommonConstant.PROCESS_I2});
		...
		map.put("pm", pm);
		log.info(thisMethodName+":end");
		return "account/finance/financeReviewList";
	}
	
	@RequestMapping("/update.do")
	public String financeReviewUpdate(Integer id, String contractId, HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		...
		log.info(thisMethodName+":end");
		return "account/finance/financeReviewUpdate";
	}
	
	@RequestMapping("/save.do")
	public void financeReviewSave(HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception  {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		...
		try {
			...
			JsonMsg jsonMsg = this.financeReviewService.operateProcess(bean,loginId,loginOrg.getOrgId());
