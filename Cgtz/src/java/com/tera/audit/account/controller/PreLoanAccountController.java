 ...
 */
@Controller
@RequestMapping("/account/preloan")
public class PreLoanAccountController extends BaseController {

	private final static Logger log = Logger.getLogger(PreLoanAccountController.class);
	
	@Autowired(required=false) //自动注入
	private ILoanBaseService loanBaseService;
	@Autowired
	private IPreLoanAccountService preLoanAccountService;
	@Autowired
	...
	public String preLoanQuery(HttpServletRequest request, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		log.info(thisMethodName+":end");
		return "account/preloan/preLoanQuery";
	}
	...
	public String preLoanList(HttpServletRequest request, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		...
		queryMap.put("bpmStates", new String[]{CommonConstant.PROCESS_H});
		...
		map.put("pm", pm);
		log.info(thisMethodName+":end");
		return "account/preloan/preLoanList";
	}
	
	@RequestMapping("/update.do")
	public String preLoanUpdate(Integer id,String contractId, HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		...
		log.info(thisMethodName+":end");
		return "account/preloan/preLoanUpdate";
	}
	
	@RequestMapping("/save.do")
	public void preLoanSave(HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception  {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
...
//		Org loginOrg = (Org) request.getSession().getAttribute(SysConstants.LOGIN_ORG);
		...
		try {
			...
			AccountJsonMsg jsonMsg = this.preLoanAccountService.collectInterest(bean,loginId);
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/applyLoan.do",method=RequestMethod.GET)
	public String applyLoan(String id, HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception  {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		...
		log.info(thisMethodName+":end");
		return "account/preloan/applyLoanUpdate";
	}
	
	@RequestMapping(value="/applyLoan.do",method=RequestMethod.POST)
	public void applyLoanSave(HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception  {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		response.setContentType("application/json;charset=UTF-8");
		PrintWriter writer = response.getWriter();
		...
		try {
			...
			JsonMsg jsonMsg = this.preLoanAccountService.operateProcess(bean,loginId,loginOrg.getOrgId());
			writer.print(JsonUtil.object2json(jsonMsg));
		} catch (Exception e) {
			log.error(thisMethodName+":error", e);
			writer.print(JsonUtil.object2json(new JsonMsg(false, "失败")));
