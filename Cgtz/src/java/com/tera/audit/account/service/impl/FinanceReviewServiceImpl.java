 ...
 *
 */
@Service("financeReviewService")
public class FinanceReviewServiceImpl implements IFinanceReviewService {
	...
	/* (non-Javadoc)
	 * @see com.tera.audit.account.service.impl.IFinanceReviewService#operateProcess(com.tera.audit.branch.model.AuditFormBean, java.lang.String, java.lang.String)
	 */
	@Override
	@Transactional
	public JsonMsg operateProcess(AuditFormBean formBean, String loginId,String orgId) throws Exception {
		...
		if ("1".equals(decision)) {
			...
			if (StringUtils.isNotNullAndEmpty(prevUser)) {
				...
				nextUser = this.commonHandlerService.getNextUser(CommonConstant.ROLE_CASH_STAFF, BusinessConstants.ORG_CODE,loanBase.getOrg());
			}
			
			if (StringUtils.isNotNullAndEmpty(nextUser)) {
				this.commonHandlerService.workFlow(bizKey, CommonConstant.PROCESS_J, nextUser, loginId, decision,remark);
				state1st = "J";
				state2nd = "0";
			} else {
		...
		if ("0".equals(decision)) {
			...
			// 到财务核帐
			if (CommonConstant.PROCESS_I1.equals(node)) {
				String prevUser = this.commonHandlerService.getPrevUser(loanId, CommonConstant.PROCESS_I1);
				this.commonHandlerService.workFlow(bizKey, CommonConstant.PROCESS_I1, prevUser, loginId,
						decision + ",退回到" + node, remark);
				state1st = "I";
				state2nd = "3";
			}
