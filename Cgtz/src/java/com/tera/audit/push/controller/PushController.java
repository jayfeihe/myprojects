package com.tera.audit.push.controller;
...
public class PushController {
	...
	private final static Logger log = Logger.getLogger(PushController.class);
	...
	public String pushQuery(HttpServletRequest request, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		log.info(thisMethodName+":end");
		return "push/pushQuery";
	}
	
	@RequestMapping("/list.do")
	public String pushList(HttpServletRequest request, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		...
		queryMap.put("bpmStates", new String[]{CommonConstant.PROCESS_N});
		queryMap.put("loanWays", new String[]{Contract.LOAN_WAY_ZQ,Contract.LOAN_WAY_ZT});
		...
		PageList<LoanBase> pushList = this.loanBaseService.queryPageList(queryMap);
		pm.setData(pushList);
	...
	public String pushUpdate(Integer id,String contractId, HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		...
		log.info(thisMethodName+":end");
		return "push/pushUpdate";
	}
	
	@RequestMapping("/save.do")
	public void pushSave(HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception  {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
...
//		Org loginOrg = (Org) request.getSession().getAttribute(SysConstants.LOGIN_ORG);
		...
		try {
			...
			JsonMsg jsonMsg = this.pushService.operateProcess(bean,loginId);
