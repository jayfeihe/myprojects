...
<%--					<th scope="col">操作</th>--%>
				...
				<c:forEach items="${ pm.data}" var="data" varStatus="status">
					<tr>
						...
						<td><a href="<c:if test="${stateType eq 'offTask' or stateType eq 'inTask' }">car/query/read.do?id=${data.id }</c:if>car/app/read.do?id=${data.id}" target="_blank">${data.appId}</a></td>
						<td>${data.name}</td>
...
function updateData(data_id,name) {
	var url="<%=basePath%>car/verify/update.do?id="+data_id;
	addTab(name, url);
	return null;
}
