...
<%--							</td>--%>
							<td align="right">渠道:
								...
								data-options="validType:['length[0,50]']" editable="false" value="${carVerify.belongChannel}"/>
							</td>
							<td align="right">产品：
								<input id="product" name="decision.product" <c:choose><c:when test="${null == carVerify.product || '' eq carVerify.product}">value='${bean.product }'</c:when><c:otherwise>value='${carVerify.product }'</c:otherwise></c:choose> type="text"  data-options="required:true,validType:['length[0,50]']" 
...
<%-- 									<input type="button" class="btn" value="补充资料" onclick="javascript:artOpenPage('影像资料','car/verify/addImgDataPopup.do?appId=${bean.appId}')"> --%>
									<br>
									退回原因:
									<textarea id="returnMsg" name="decision.returnMsg" value="${carVerify.returnMsg}" class="textbox easyui-validatebox"
									data-options="required:true,validType:['length[0,500]']" 
									style="resize: none;width:390px;height:50px!important;vertical-align:middle;background-color: #F0F0F0;" readonly="readonly">${carVerify.returnMsg}</textarea>
								</div>
...
<%--						</tr>--%>
						...
						<tr>
							...
							<td colspan="5">
								...
								style="resize: none;width:625px;height:50px!important;background-color: #F0F0F0;" readonly="readonly">${carVerify.remarks}</textarea>
							</td>
						</tr>
						<tr>
							<td colspan="6">
