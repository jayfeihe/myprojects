				...
				<c:if test="${not empty carFraud }">
			 	...
			 	<tr>
					<td colspan="9">
						<div style="font-size: 14px;" >
							<strong>审核</strong>
								<input type="hidden" name="decision.id" value="${carVerify.id}" />
			 	...
			 	<tr><td>
			 		<table id="verifyInfo">
			 			<tr>
							<td align="right">借款金额:</td>
							<td><div id ="amountDiv">
							<input id="amount" name="decision.amount" value="${carVerify.amount/10000}" type="text"
...
<%--							<td ><input id="period" name="decision.period" value="${carVerify.period}" type="text" data-options="required:true,validType:['length[0,10]']" --%>
...
<%--							</td>--%>
							<td align="right">渠道:
								...
								data-options="required:true,validType:['length[0,50]']" editable="false" value="${carVerify.belongChannel}"/>
							</td>
							<td align="right">产品：
								<input id="product" name="decision.product" value="${carVerify.product }" type="text"  data-options="required:true,validType:['length[0,50]']" 
								class="easyui-combobox" editable="false" style="width:152px;"/>
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td>
								<input id="decision" name="decision.decision" value="${carVerify.decision}" type="text"  data-options="required:true,validType:['length[0,50]']" 
								class="easyui-combobox" editable="false" style="width:152px;"/>
							</td>
							<td align="center" colspan="4" valign="top">
								<div id="noPassReasonDiv" >
									...
									<input id="decisionCode5" name="decision.decisionCode5" 
										value="${carVerify.decisionCode5}" type="text" 
										data-options="validType:['length[0,50]']" 
										class="easyui-combobox" editable="false" style="width:152px;"/>
									<input id="decisionCode6" name="decision.decisionCode6" 
										value="${carVerify.decisionCode6}" type="text" 
										data-options="validType:['length[0,50]']" 
										class="easyui-combobox" editable="false" style="width:152px;"/>
									
<%--								<input type="button" class="btn" value="补充资料" onclick="addObj('commonContacts');"/>--%>
<%-- 									<input type="button" class="btn" value="补充资料" onclick="javascript:artOpenPage('影像资料','car/verify/addImgDataPopup.do?appId=${bean.appId}')"> --%>
									<br>
									退回原因:
									<textarea id="returnMsg" name="decision.returnMsg" value="${carVerify.returnMsg}" class="textbox "
									data-options="required:true,validType:['length[0,500]']" 
									style="resize: none;width:390px;height:50px!important;vertical-align:middle;">${carVerify.returnMsg}</textarea>
								</div>
...
<%--						</tr>--%>
						...
						<tr>
							<td>备注：</td>
							<td colspan="5">
								<textarea id="remarks" name="decision.remarks" value="${carVerify.remarks}"
								class="textbox easyui-validatebox" data-options="validType:['length[0,100]']"
								style="resize: none;width:625px;height:50px!important;">${carVerify.remarks}</textarea>
							</td>
						</tr>
						<tr>
							<td colspan="6">
...
// 	$.ajax({
// 		type : "POST",
<%-- 		url : "<%=basePath%>car/antifraud/update.do", --%>
// 		data : params+"&buttonType="+buttonType,
// 		success : function(data) {
// 			//关闭遮罩，弹出消息框
// 			window.parent.closeMask();
...
function updateFunction(buttonType) {
	...
	$.ajax({
		type : "POST",
		url : "<%=basePath%>car/verify/save.do",
		data : params+"&buttonType="+buttonType,
		success : function(data) {
			//关闭遮罩，弹出消息框
			window.parent.closeMask();
...
function updateYhkje(){
	if($('#verifyInfo').find('#amount').val() != '' && $('#verifyInfo').find('#product').combobox('getValue') != ''){
		updateJe("<%=basePath%>" + "car/verify/getYhkje.do", "fkje=" + $('#verifyInfo').find('#amount').val() + "&productName=" + $('#verifyInfo').find('#product').combobox('getValue'), "yhkje");
	}
}
...
function updateFkje(){
	if($('#verifyInfo').find('#yhkje').val() != '' && $('#verifyInfo').find('#product').combobox('getValue') != ''){
		updateJe("<%=basePath%>" + "car/verify/getFkje.do", "yhkje=" + $('#verifyInfo').find('#yhkje').val() + "&productName=" + $('#verifyInfo').find('#product').combobox('getValue'), "fkje");
	}
}
...
//填充select数据 解除反欺诈结果
	$("#carAntifraud").find("#result").combobox("clear");
	$("#carAntifraud").find('#result').combobox({
		valueField:'keyProp', 
		textField:'keyValue',
...
//填充select数据 借款期限
	...
	$("#verifyInfo").find('#belongChannel').combobox({
		...
		onChange: function(newValue, oldValue){             
			...
			var producturl1 = "product/hedao/listjason.do?type=4&state=1&flag=1&belongChannel=" + encodeURI(newValue);
			$("#verifyInfo").find('#product').combobox('reload',producturl1); 
		}
	});
	//填充select数据 产品
	var belongChannel1 = $("#verifyInfo").find('#belongChannel').combobox('getValue');
	var producturl1 = "product/hedao/listjason.do?type=4&state=1&flag=1&belongChannel=" + encodeURI(belongChannel1);
	$("#verifyInfo").find("#product").combobox("clear");
