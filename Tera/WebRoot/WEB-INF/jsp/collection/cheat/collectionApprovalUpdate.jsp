				...
				<form id="updateForm">
					<table >
						...
						<tr>
							...
							<td>
							<c:choose>
								<c:when test="${bean.checkResult=='1'}">
									<input id="checkResult" name="checkResult" type="text" disabled="disabled"
									class="textbox" value="通过" />
								</c:when>
								<c:when test="${bean.checkResult=='2'}">
									<input id="checkResult" name="checkResult" type="text" disabled="disabled"
									class="textbox" value="退回" />
								</c:when>
							</c:choose>	
							</td>
						</tr>	
...
function updateFunction() {
	...
	$.ajax({
		type : "POST",
		url : "<%=basePath%>" + "collection/cheat/approval/save.do",
		...
		success : function(data) {
			...
			if ("true"==data.success) {
				$.messager.alert('消息', data.message, 'info', function(){
						window.location="<%=basePath%>collection/cheat/approval/query.do";
					});
				} else {
