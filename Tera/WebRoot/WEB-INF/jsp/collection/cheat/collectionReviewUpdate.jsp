				...
				<form id="updateForm">
					<table >
						...
						<tr>
								...
								<c:if test="${bean.collectionWay=='1'}">
								<option value="3">提交强制落地</option>
								</c:if>
								</select>	
								</td>
							</tr>
...
function updateFunction() {
	...
	$.ajax({
		type : "POST",
		url : "<%=basePath%>" + "collection/cheat/review/save.do",
		data : params,
		success : function(data) {
			...
			if ("true"==data.success) {
				$.messager.alert('消息', data.message, 'info', function(){
						window.location="<%=basePath%>collection/cheat/review/query.do";
					});
				} else {
