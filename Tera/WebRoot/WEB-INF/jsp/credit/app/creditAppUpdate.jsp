...
<div id="main">
	<div id="part1" class="part">
		<p class="title"><a href="javascript:void(0);">个人信用借款申请</a></p>
		<div class="content">
			<form id="updateForm" name="creditApp.updateForm" action="creditApp/update.do" AUTOCOMPLETE="off">
				<input id="appId" name="creditApp.appId" type="hidden" value="${bean.appId}"/>
				<input id="customerId" name="creditApp.customerId" type="hidden" value="${bean.customerId}"/>
				<input id="id" name="creditApp.id" type="hidden" size="35" value="${bean.id}" />
				<table id="appInfo">
					<div style="margin-left: 10px;margin-top: 10px; font-size: 14px;"><strong>借款信息</strong></div><hr/>
					<tr>
						<td>客户来源:</td>
						<td><input id="customerSource" name="creditApp.customerSource" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox"  value="${bean.customerSource}" editable="false"/></td>
						<td>营销人员:</td>
						<td><input id="staffNo" name="creditApp.staffNo" type="text" data-options="required:true,validType:['length[0,50]']" 
						class="easyui-combobox"  value="${bean.staffNo}" editable="false"/></td>
						<td>申请编码:</td>
						<td><input id="appCode" name="creditApp.appCode" type="text" data-options="validType:['length[0,30]']" 
						class="textbox easyui-validatebox" value="${bean.appCode}"/></td>
					</tr>
					<tr>
						<td align="right">渠道:</td>
						<td><input id="belongChannel" name="creditApp.belongChannel" value="${bean.belongChannel}" type="text" class="easyui-combobox" data-options="required:true,validType:['length[0,50]']" editable="false"/></td>
						<td align="right">产品:</td>
						<td><input id="product" name="creditApp.product" type="text" data-options="required:true,validType:['length[0,50]']" 
						class="easyui-combobox"  value="${bean.product}" editable="false"/></td>
					</tr>
					<tr>
						<td>借款金额:</td>
						<td><input id="amount" name="creditApp.amount" type="text" editable="false"  data-options="required:true,min:0,precision:2" 
						class="textbox easyui-numberbox" value="${bean.amount/10000}" style="width:128px;"/>万元</td>
<%--						<td>借款期限:</td>--%>
<%--						<td><input id="period" name="creditApp.period" type="text" editable="false"  data-options="required:true,min:0,precision:0" --%>
<%--						class="easyui-combobox"  value="${bean.period}" editable="false"/></td>--%>
						<td>借款用途:</td>
						<td colspan="3"><input id="useage1" name="creditApp.useage1" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox"  value="${bean.useage1}" editable="false"/>
						<input id="useage2" name="creditApp.useage2" type="text" data-options="required:true,validType:['length[0,10]']" 
				...
				<table id="basicInfo">
					...
					<tr>
						...
						<td><input id="name" name="creditApp.name" type="text" data-options="required:true,validType:['length[0,50]']" 
							class="textbox easyui-validatebox" value="${bean.name}"/></td>
						<td>身份证号:</td>
						<td>
<%--						<input id="idType" name="creditApp.idType" type="hidden" value="01" />--%>
						<input id="idNo" name="creditApp.idNo" onChange="getCustomer();" type="text" data-options="required:true,validType:['idcard']" 
						class="textbox easyui-validatebox" value="${bean.idNo}"/></td>
						<td><SPAN style="color:red">*</SPAN>手机号:</td>
						<td><input id="mobile" name="creditApp.mobile" type="text" data-options="required:true,validType:['mobile']" 
						class="textbox easyui-numberbox" value="${bean.mobile}"/></td>
						<td>学历:</td>
						<td><input id="diploma" name="creditApp.diploma" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox" value="${bean.diploma}" editable="false"/></td>
					</tr>	
					<tr>	
						<td>微信:</td>
						<td><input id="wechat" name="creditApp.wechat" type="text" data-options="validType:['length[0,50]']" 
						class="textbox easyui-validatebox" value="${bean.wechat}"/></td>
						<td>邮箱:</td>
						<td><input id="email" name="creditApp.email" type="text" data-options="validType:['email','length[0,50]']" 
						class="textbox easyui-validatebox" value="${bean.email}"/></td>
						<td>子女信息:</td>
						<td>
							<input id="withoutChildren" name="creditApp.withoutChildren" 
							...
							<input id="childrenSize" name="creditApp.childrenSize" type="text" editable="false"  data-options="min:0,precision:0" 
							class="textbox easyui-numberbox" value="${bean.childrenSize}" 
							style="width:52px;"/>个
						</td>
						<td><SPAN style="color:red">*</SPAN>婚姻状况:</td>
						<td><input id="marriage" name="creditApp.marriage" type="text" data-options="required:true,validType:['length[0,2]']" 
						class="easyui-combobox" value="${bean.marriage}" editable="false"/></td>
					</tr>
					<tr>
						<td>户籍:</td>
						<td colspan="3"><input id="kosekiProvince" name="creditApp.kosekiProvince" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox" value="${bean.kosekiProvince}" editable="false" style="width:140px;" />省
						<input id="kosekiCity" name="creditApp.kosekiCity" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox" value="${bean.kosekiCity}" editable="false" style="width:140px;" />市</td>
						<td>居住性质:</td>
						<td><input id="addNature" name="creditApp.addNature" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox" value="${bean.addNature}" editable="false"/></td>
						<td>居住时间:</td>
						<td><input id="addYear" name="creditApp.addYear" type="text" editable="false"  data-options="required:true,min:0,precision:0" 
						class="textbox easyui-numberbox" value="${bean.addYear}" style="width:140px;"/>年</td>	
					</tr>
					<tr>
						<td>居住地址:</td>
						<td colspan="5"><input id="addProvince" name="creditApp.addProvince" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox" value="${bean.addProvince}" editable="false" style="width:140px;"/>省
						<input id="addCity" name="creditApp.addCity" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox" value="${bean.addCity}" editable="false" style="width:140px;"/>市
						<input id="addCounty" name="creditApp.addCounty" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox" value="${bean.addCounty}" editable="false" style="width:128px;"/>区县
						<input id="address" name="creditApp.address" type="text" data-options="required:true,validType:['length[0,100]']" 
						class="textbox easyui-validatebox" value="${bean.address}" editable="false"/></td>
					</tr>
					<tr>
						<td>住宅电话:</td>
						<td><input id="phone" name="creditApp.phone" type="text" data-options="required:true,validType:['length[0,50]']" 
						class="textbox easyui-validatebox" value="${bean.phone}"/></td>
						<td>单位名称:</td>
						<td><input id="comName" name="creditApp.comName" type="text" data-options="required:true,validType:['length[0,50]']" 
						class="textbox easyui-validatebox" value="${bean.comName}"/></td>
						<td>单位类型:</td>
						<td><input id="comType" name="creditApp.comType" type="text" data-options="required:true,validType:['length[0,50]']" 
						class=easyui-combobox value="${bean.comType}" editable="false"/></td>
						<td>单位电话:</td>
						<td><input id="comPhone" name="creditApp.comPhone" type="text" data-options="required:true,validType:['length[0,50]']" 
						class="textbox easyui-validatebox" value="${bean.comPhone}"/></td>
					</tr>
					<tr>
						<td>单位地址:</td>
						<td colspan="5"><input id="comProvince" name="creditApp.comProvince" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox" value="${bean.comProvince}" editable="false" style="width:140px;"/>省
						<input id="comCity" name="creditApp.comCity" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox" value="${bean.comCity}" editable="false" style="width:140px;"/>市
						<input id="comCounty" name="creditApp.comCounty" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox" value="${bean.comCounty}" editable="false" style="width:130px;"/>区县
						<input id="comAddress" name="creditApp.comAddress" type="text" data-options="required:true,validType:['length[0,100]']" 
						class="textbox easyui-validatebox" value="${bean.comAddress}" editable="false"/></td>
					</tr>
					<tr>
						<td>单位担任职务:</td>
						<td><input id="comDuty" name="creditApp.comDuty" type="text" data-options="required:true,validType:['length[0,50]']" 
						class="textbox easyui-validatebox" value="${bean.comDuty}"/></td>
						<td>入职时间:</td>
						<td><input id="comAddDate" name="creditApp.comAddDate" type="text" editable="false" data-options="required:true" 
						class="textbox easyui-datebox" value="${bean.comAddDateStr}"/></td>
						<td>年收入:</td>
						<td><input id="income" name="creditApp.income" type="text" editable="false"  data-options="required:true,min:0,precision:2" 
						class="textbox easyui-numberbox" value="${bean.income/10000}" style="width:128px;"/>万元</td>
						<td>月还款额:</td>
						<td><input id="monthlyPayments" name="creditApp.monthlyPayments" type="text" editable="false"  data-options="required:true,min:0,precision:2" 
						class="textbox easyui-numberbox" value="${bean.monthlyPayments}" style="width:130px;"/>元</td>
					</tr>
					<tr>
						<td>开户行名称:</td>
						<td><input id="accName" name="creditApp.accName" type="text" data-options="required:true,validType:['length[0,100]']" 
						class="textbox easyui-validatebox" value="${bean.accName}"/></td>
						<td>开户行帐号:</td>
						<td><input id="account" name="creditApp.account" type="text" data-options="required:true,validType:['number','length[0,100]']"
			...
			<div id = "marriageDiv" <c:if test="${bean.marriage!='02'}">style="display: none;"</c:if>>
				<table id="spouse">	
					...
					<tr>
						<td><SPAN style="color:red">*</SPAN>姓名:</td>
						<td><input id="spouseName" name="creditApp.spouseName" type="text" data-options="required:true,validType:['length[0,50]']" 
						class="textbox easyui-validatebox" value="${bean.spouseName}"/></td>
						<td>身份证号:</td>
						<td>
<%--						<input id="spouseIdType" name="creditApp.spouseIdType" type="hidden" value="01" />--%>
						<input id="spouseIdNo" name="creditApp.spouseIdNo" type="text" data-options="required:true,validType:['idcard']" 
						class="textbox easyui-validatebox" value="${bean.spouseIdNo}"/></td>
						<td><SPAN style="color:red">*</SPAN>手机:</td>
						<td><input id="spouseMobile" name="creditApp.spouseMobile" type="text" data-options="required:true,validType:['mobile']" 
						class="textbox easyui-numberbox" value="${bean.spouseMobile}"/></td>
					</tr>
					<tr>
						<td>单位名称:</td>
						<td><input id="spouseComName" name="creditApp.spouseComName" type="text" data-options="required:true,validType:['length[0,50]']" 
						class="textbox easyui-validatebox" value="${bean.spouseComName}"/></td>
						<td>年收入:</td>
						<td><input id="spouseIncome" name="creditApp.spouseIncome" type="text" editable="false"  data-options="required:true,min:0,precision:2" 
						class="textbox easyui-numberbox" value="${bean.spouseIncome/10000}" style="width:128px;"/>万元</td>
						<td>职业:</td>
						<td><input id="spouseJob" name="creditApp.spouseJob" type="text" data-options="required:true,validType:['length[0,50]']" 
						class="textbox easyui-validatebox" value="${bean.spouseJob}"/></td>
					</tr>
					<tr>
						<td>单位地址:</td>
						<td colspan="5"><input id="spouseComProvince" name="creditApp.spouseComProvince" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox" value="${bean.spouseComProvince}" editable="false" style="width:140px;" />省
						<input id="spouseComCity" name="creditApp.spouseComCity" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox" value="${bean.spouseComCity}" editable="false" style="width:140px;" />市
						<input id="spouseComCounty" name="creditApp.spouseComCounty" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox" value="${bean.spouseComCounty}" editable="false" style="width:128px;" />区县
						<input id="spouseComAddress" name="creditApp.spouseComAddress" type="text" data-options="required:true,validType:['length[0,100]']" 
...
<div id="mainBusinessInfoDiv" <c:if test="${fn:contains(bean.product,'业主贷')==false}">style="display: none;"</c:if>>
	<table>	
		...
		<tr>
			<td>商号名称:</td>
			<td><input id="firmName" name="creditApp.firmName" type="text" data-options="required:true,validType:['length[0,100]']" 
			class="textbox easyui-validatebox" value="${bean.firmName}"/></td>
			<td>主营业务:</td>
			<td><input id="firmMainBusiness" name="creditApp.firmMainBusiness" type="text" data-options="required:true,validType:['length[0,100]']" 
			class="textbox easyui-validatebox" value="${bean.firmMainBusiness}"/></td>
			<td>年营业额:</td>
			<td><input id="firmIncome" name="creditApp.firmIncome" type="text" editable="false"  data-options="required:true,min:0,precision:2" 
			class="textbox easyui-numberbox" value="${bean.firmIncome/10000}" style="width:128px;"/>万元</td>
			<td>占股比例:</td>
			<td><input id="firmShare" name="creditApp.firmShare" type="text" editable="false"  data-options="required:true,min:0,precision:2" 
			class="textbox easyui-numberbox" value="${bean.firmShare}" style="width:140px;"/>%</td>				
		</tr>
		<tr>
			<td>商号地址:</td>
			<td colspan="5" ><input id="firmProvince" name="creditApp.firmProvince" type="text" data-options="required:true,validType:['length[0,10]']" 
			class="easyui-combobox" value="${bean.firmProvince}" editable="false" style="width:140px;" />省
			<input id="firmCity" name="creditApp.firmCity" type="text" data-options="required:true,validType:['length[0,10]']" 
			class="easyui-combobox" value="${bean.firmCity}" editable="false" style="width:140px;" />市
			<input id="firmCounty" name="creditApp.firmCounty" type="text" data-options="required:true,validType:['length[0,10]']" 
			class="easyui-combobox" value="${bean.firmCounty}" editable="false" style="width:128px;" />区县
			<input id="firmAddress" name="creditApp.firmAddress" type="text" data-options="required:true,validType:['length[0,100]']" 
			class="textbox easyui-validatebox" value="${bean.firmAddress}" editable="false"/></td>
		</tr>
		<tr>
			<td>本地经营时间:</td>
			<td><input id="firmManageDate" name="creditApp.firmManageDate" type="text" editable="false" data-options="required:true"
			...
			<img src="img/deleteItem2.png"  onclick="rmObj('dealingsContact_dealingsContactsIndex');" />
				...
				<table>	
					...
					<tr>
						<td>是否有未结清借款:</td>
						<td id="creditExt_1">
							<input type="hidden" name="creditExts[0].id" value="${creditExts[0].id}" />
							<input type="hidden" name="creditExts[0].appId" value="${bean.appId}" />
							<input type="hidden" name="creditExts[0].type" value="1" />
							<input type="hidden" name="creditExts[0].key" value="1" />
							<input type="hidden" name="creditExts[0].name" value="是否有未结清借款" />
							<input type="radio" id="value" name="creditExts[0].value" value='0'
								<c:if test="${creditExts[0].value=='0'||creditExts[0].value==null}">checked='checked'</c:if> />否
							<input type="radio" id="entLoanMark" name="creditExts[0].value" value="1"
								<c:if test="${creditExts[0].value=='1'}">checked='checked'</c:if> />是
							<input id="remarks" name="creditExts[0].remarks" type="text" data-options="min:0,precision:0,validType:['length[0,500]']" 
							class="textbox easyui-validatebox" value="${creditExts[0].remarks}"/>
						</td>
											
						<td>是否有资产已被抵押:</td>
						<td id="creditExt_2">
							<input type="hidden" name="creditExts[1].id" value="${creditExts[1].id}" />
							<input type="hidden" name="creditExts[1].appId" value="${bean.appId}" />
							<input type="hidden" name="creditExts[1].type" value="1" />
							<input type="hidden" name="creditExts[1].key" value="2" />
							<input type="hidden" name="creditExts[1].name" value="是否有资产已被抵押" />
							<input type="radio" id="value" name="creditExts[1].value" value='0'
								<c:if test="${creditExts[1].value=='0'||creditExts[1].value==null}">checked='checked'</c:if> />否
							<input type="radio" id="entLoanMark" name="creditExts[1].value" value="1"
								<c:if test="${creditExts[1].value=='1'}">checked='checked'</c:if> />是
							<input id="remarks" name="creditExts[1].remarks" type="text" data-options="min:0,precision:0,validType:['length[0,500]']" 
							class="textbox easyui-validatebox" value="${creditExts[1].remarks}"/>
						</td>
					</tr>
					<tr>
						<td>是否涉及诉讼:</td>
						<td id="creditExt_3">
							<input type="hidden" name="creditExts[2].id" value="${creditExts[2].id}" />
							<input type="hidden" name="creditExts[2].appId" value="${bean.appId}" />
							<input type="hidden" name="creditExts[2].type" value="1" />
							<input type="hidden" name="creditExts[2].key" value="3" />
							<input type="hidden" name="creditExts[2].name" value="是否涉及诉讼" />
							<input type="radio" id="value" name="creditExts[2].value" value='0'
								<c:if test="${creditExts[2].value=='0'||creditExts[2].value==null}">checked='checked'</c:if> />否
							<input type="radio" id="entLoanMark" name="creditExts[2].value" value="1"
								<c:if test="${creditExts[2].value=='1'}">checked='checked'</c:if> />是
							<input id="remarks" name="creditExts[2].remarks" type="text" data-options="min:0,precision:0,validType:['length[0,500]']" 
							class="textbox easyui-validatebox" value="${creditExts[2].remarks}"/>
						</td>			
						<td>近期是否有对生意有较大影响的事件:</td>
						<td id="creditExt_4">
							<input type="hidden" name="creditExts[3].id" value="${creditExts[3].id}" />
							<input type="hidden" name="creditExts[3].appId" value="${bean.appId}" />
							<input type="hidden" name="creditExts[3].type" value="1" />
							<input type="hidden" name="creditExts[3].key" value="4" />
							<input type="hidden" name="creditExts[3].name" value="近期是否有对生意有较大影响的事件" />
							<input type="radio" id="value" name="creditExts[3].value" value='0'
								<c:if test="${creditExts[3].value=='0'||creditExts[3].value==null}">checked='checked'</c:if> />否
							<input type="radio" id="entLoanMark" name="creditExts[3].value" value="1"
								<c:if test="${creditExts[3].value=='1'}">checked='checked'</c:if> />是
							<input id="remarks" name="creditExts[3].remarks" type="text" data-options="min:0,precision:0,validType:['length[0,500]']" 
							class="textbox easyui-validatebox" value="${creditExts[3].remarks}"/>
						</td>
					</tr>
					<tr>
						<td>是否为其他人提供担保:</td>
						<td id="creditExt_5">
							<input type="hidden" name="creditExts[4].id" value="${creditExts[4].id}" />
							<input type="hidden" name="creditExts[4].appId" value="${bean.appId}" />
							<input type="hidden" name="creditExts[4].type" value="1" />
							<input type="hidden" name="creditExts[4].key" value="5" />
							<input type="hidden" name="creditExts[4].name" value="是否为其他人提供担保" />
							<input type="radio" id="value" name="creditExts[4].value" value='0'
								<c:if test="${creditExts[4].value=='0'||creditExts[4].value==null}">checked='checked'</c:if> />否
							<input type="radio" id="entLoanMark" name="creditExts[4].value" value="1"
								<c:if test="${creditExts[4].value=='1'}">checked='checked'</c:if> />是
							<input id="remarks" name="creditExts[4].remarks" type="text" data-options="min:0,precision:0,validType:['length[0,500]']" 
							class="textbox easyui-validatebox" value="${creditExts[4].remarks}"/>
						</td>			
						<td>本人或配偶是否有“曾用名”:</td>
						<td id="creditExt_6">
							<input type="hidden" name="creditExts[5].id" value="${creditExts[5].id}" />
							<input type="hidden" name="creditExts[5].appId" value="${bean.appId}" />
							<input type="hidden" name="creditExts[5].type" value="1" />
							<input type="hidden" name="creditExts[5].key" value="6" />
							<input type="hidden" name="creditExts[5].name" value="本人或配偶是否有“曾用名”" />
							<input type="radio" id="value" name="creditExts[5].value" value='0'
								<c:if test="${creditExts[5].value=='0'||creditExts[5].value==null}">checked='checked'</c:if> />否
							<input type="radio" id="entLoanMark" name="creditExts[5].value" value="1"
								<c:if test="${creditExts[5].value=='1'}">checked='checked'</c:if> />是
							<input id="remarks" name="creditExts[5].remarks" type="text" data-options="min:0,precision:0,validType:['length[0,500]']" 
							class="textbox easyui-validatebox" value="${creditExts[5].remarks}"/>
						</td>
					</tr>
					<tr>
						<td>
						...
						<td colspan="3">
							<textarea id="remark" name="creditApp.remark" value="${bean.remark}" class="textbox easyui-validatebox" data-options="required:true,validType:['length[0,500]']" 
					...
					<tr>
						<td colspan=3">
							...
							<input type="button" value="前端拒贷" class="btn" onclick="javascript:artOpenPage('前端拒贷','credit/app/frontRefusePopup.do?id=${bean.id}');" title="拒贷"/>&nbsp;&nbsp;
...
function updateFunction(buttonType) {
	...
	$.ajax({
		type : "POST",
		url : "<%=basePath%>" + "credit/app/save.do",
		...
		success : function(data) {
			...
			if ("true"==data.success) {
				$.messager.alert('消息', data.message, 'info', function(){
					...
					if(buttonType=="save") {
						location.replace("<%=basePath%>credit/app/update.do?id="+data.id);
					}else{
						window.history.go(-1);
					}
            	});
...
$(document).ready(function (){
	...
	$('#belongChannel').combobox({
		...
		onChange: function(newValue, oldValue){             
			...
			var producturl = "product/hedao/listjason.do?type=3&state=1&belongChannel=" + encodeURI(newValue);
			$('#product').combobox('reload',producturl); 
		}
	});
	//填充select数据 产品
	var belongChannel = $('#belongChannel').combobox('getValue');
	var producturl = "product/hedao/listjason.do?type=3&state=1&belongChannel=" + encodeURI(belongChannel);
	$("#product").combobox("clear");
	$('#product').combobox({
		url:producturl,
		valueField:'name', 
