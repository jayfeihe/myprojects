			...
			<table id="table" class="datatable" summary="list of members in EE Studay">
				...
				<c:forEach items="${ pm.data}" var="data" varStatus="status">
					<tr>
						...
						<td><a href="<c:if test="${stateType eq 'offTask' }">credit/query/read.do?id=${data.id }</c:if>credit/app/read.do?id=${data.id}" target="_blank">${data.appId}</a></td>
						<td>${data.name}</td>
						...
						<td>${data.product}</td>
						<td>${data.creditDecisionOperator}</td>
						<td>
							<c:choose>
								<c:when test="${data.creditDecisionCode=='00'}">退回</c:when>
								<c:when test="${data.creditDecisionCode=='02'}">拟通过</c:when>
								<c:when test="${data.creditDecisionCode=='03'}">拟拒贷</c:when>
							</c:choose>	
						</td>
						<td>
							<c:choose>
						...
						<c:if test="${stateType=='waitTask'}">
						<td>
							<a href="javascript:void(0);" onclick="javascript:updateData('${ data.id}','${data.name}');">审批</a>&nbsp;
						</td>
						</c:if>
						<c:if test="${stateType=='inTask'||stateType=='offTask'}">
						<td>
...
function updateData(data_id,name) {
    var url="<%=basePath%>credit/approval/update.do?id="+data_id;
    addTab(name, url);
    return null;
}  
