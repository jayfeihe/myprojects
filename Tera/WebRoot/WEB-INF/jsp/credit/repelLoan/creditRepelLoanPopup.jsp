...
<%--	<div style="width: 350px;height: 200px" name="inputInfo">--%>
    ...
    <table>
		...
		<tr> 
			<td>
			 	拒件码:<br/>
			    <input id="decisionCode3" name="decisionCode3" value="" type="text" editable="false"
			    data-options="required:true,validType:['length[0,50]']" class="easyui-combobox" />
			    <input id="decisionCode4" name="decisionCode4" value="" type="text" editable="false"
			    data-options="required:true,validType:['length[0,50]']" class="easyui-combobox" />
			</td>
		</tr>
		<tr> 
			<td>
			 	 反馈销售描述:
			    <textarea id="feedbackDescribe" name="feedbackDescribe" data-options="required:true" style="resize: none;width:100%;height:50px !important;" class="textbox easyui-validatebox"></textarea>
			</td> 
		</tr>
		<tr> 
			<td>
...
function repelLoanFn(id) {
	...
	$.messager.confirm('消息', "您确认要拒贷吗？", function(ok){
		if (ok){
			openMask();
			$.ajax({
				url: "credit/repelLoan/repelLoan.do",
