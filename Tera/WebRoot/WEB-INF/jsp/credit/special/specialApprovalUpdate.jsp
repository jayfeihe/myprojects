...
<%--								</td>--%>
						...
						<tr>
							<td id="creditDecision_1">
								...
								<c:if test="${creditDecision1.decision eq '00'}">
									<div id ="noPassReasonDiv1">
										退回原因:<textarea value="${creditDecision1.returnMsg}" class="textbox easyui-validatebox"
											data-options="validType:['length[0,500]']" 
											style="resize: none;width:325px;height:50px!important;vertical-align:middle;background-color: #F0F0F0;" disabled="disabled">${creditDecision1.returnMsg}</textarea>
									</div>
								</c:if>
								<c:if test="${creditDecision1.decision eq '03'}">
						...
						<tr>
							<td>
								...
								<td colspan="5">
									...
									style="font-size:12px;resize: none;width:625px;height:50px!important;background-color: #F0F0F0;" disabled="disabled">${creditDecision1.remarks}</textarea>
								</td>
							</td>							
						</tr>
					</table>
...
<%--								</td>--%>
						...
						<tr>
							<td>
								...
								<td align="right" colspan="4">
									<c:if test="${creditDecision2.decision eq '00'}">
										<div id ="noPassReasonDiv2">
											退回原因:<textarea value="${creditDecision2.returnMsg}" class="textbox easyui-validatebox"
												data-options="validType:['length[0,500]']" 
												style="resize: none;width:325px;height:50px~important;vertical-align:middle;background-color: #F0F0F0;" disabled="disabled">${creditDecision2.returnMsg}</textarea>
										</div>
									</c:if>
									<c:if test="${creditDecision2.decision eq '04'}">
									...
									<c:if test="${creditDecision2.decision eq '05'}">
...
<%--								</td>--%>
								<td align="right">渠道:
									...
									data-options="required:true,validType:['length[0,50]']" editable="false" value="${creditDecision3.belongChannel}"/>
								</td>
								<td align="right">产品：
									<input id="product" name="decision.product" type="text" value="${creditDecision3.product }"
						...
						<tr>
							<td id="creditDecision_1">
								...
								<td>
									<select id="decision1" name="decision.decision" value="${creditDecision3.decision}"
										data-options="validType:['length[0,50]']
										,onChange: function(decisionVal){
								...
								<td align="right" colspan="4">
									<div id ="noPassReasonDiv">
										退回原因:<textarea id="returnMsg" name="decision.returnMsg" value="${creditDecision3.returnMsg}" class="textbox easyui-validatebox"
											data-options="required:true,validType:['length[0,500]']" 
											style="resize: none;width:325px;height:50px!important;vertical-align:middle;">${creditDecision3.returnMsg}</textarea>
									</div>
						...
						<tr>
							<td id="creditDecision_2">
								...
								<td colspan="5">
									...
									style="resize: none;width:625px;height:50px!important;">${creditDecision3.remarks}</textarea>
								</td>
							</td>								
						</tr>
						<tr>
...
function updateFunction() {
	...
	$.ajax({
		type : "POST",
		url : "<%=basePath%>" + "credit/decision/save.do",
		...
		success : function(data) {
			...
			if ("true"==data.success) {
				$.messager.alert('消息', data.message, 'info', function(){
//	                var url= "<%=basePath%>" + "credit/special/query.do";
//					window.location=url;
					//window.history.go(-1);
					window.parent.removeTab();
            	});
...
function saveOrSubForm(buttonType){
	...
	$.ajax({
		type : "POST",
		url : "<%=basePath%>" + "credit/special/save.do",
		data : params+"&buttonType="+buttonType,
		success : function(data) {
			//关闭遮罩，弹出消息框
			window.parent.closeMask();
...
function updateYhkjeHtje(){
	if($('#specialApprovalInfo').find('#amount').val() != '' && $('#specialApprovalInfo').find('#product').combobox('getValue') != ''){
		updateJe("<%=basePath%>" + "credit/verify/getJe.do", "yhkje=0.00&htje=0.00&jkje=" + $('#specialApprovalInfo').find('#amount').val() + "&productName=" + $('#specialApprovalInfo').find('#product').combobox('getValue'), "jkje");
	}
}
...
function updateJkjeHtje(){
	if($('#specialApprovalInfo').find('#yhkje3').val() != '' && $('#specialApprovalInfo').find('#product').combobox('getValue') != ''){
		updateJe("<%=basePath%>" + "credit/verify/getJe.do", "jkje=0.00&htje=0.00&yhkje=" + $('#specialApprovalInfo').find('#yhkje3').val() + "&productName=" + $('#specialApprovalInfo').find('#product').combobox('getValue'), "yhkje");
	}
}
...
function updateJkjeYhkje(){
	if($('#specialApprovalInfo').find('#htje').val() != '' && $('#specialApprovalInfo').find('#product').combobox('getValue') != ''){
		updateJe("<%=basePath%>" + "credit/verify/getJe.do", "jkje=0.00&yhkje=0.00&htje=" + $('#specialApprovalInfo').find('#htje').val() + "&productName=" + $('#specialApprovalInfo').find('#product').combobox('getValue'), "htje");
	}
}
...
$(document).ready(function (){
	...
	if('${creditDecision3.decision}' == '' || '${creditDecision3.decision}' == '01' || '${creditDecision3.decision}' == '06'){
		$('#noPassReasonDiv').hide();//隐藏div 
		toggleValidate('#noPassReasonDiv',false);
		$('#returnCodeDiv').hide();//隐藏div 
		toggleValidate('#returnCodeDiv',false);
...
*/
	...
	$("#specialApprovalInfo").find('#belongChannel').combobox({
		...
		onChange: function(newValue, oldValue){             
			...
			var producturl3 = "product/hedao/listjason.do?type=3&state=1&belongChannel=" + encodeURI(newValue);
	...
	var producturl3 = "product/hedao/listjason.do?type=3&state=1&belongChannel=" + encodeURI(belongChannel3);
	$("#specialApprovalInfo").find("#product").combobox("clear");
