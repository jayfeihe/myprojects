		...
		<c:forEach items="${ pm.data}" var="data" varStatus="status">
			<tr>
				...
				<c:if test="${data.currentBpmState eq '录入申请' }">
					<td><a href="credit/app/read.do?id=${data.id}" target="_blank">查看重复人详情</a></td>
				</c:if>
				<c:if test="${data.currentBpmState eq '审核' }">
					<td><a href="credit/verify/read.do?id=${data.id}" target="_blank">查看重复人详情</a></td>
				</c:if>
				<c:if test="${data.currentBpmState eq '审批' }">
					<td><a href="credit/query/read.do?id=${data.id}" target="_blank">查看重复人详情</a></td>
				</c:if>
			</tr>
		</c:forEach>
	</table>   
