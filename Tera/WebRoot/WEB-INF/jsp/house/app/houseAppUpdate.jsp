...
<div id="main">
	<div id="part1" class="part">
		...
		<div class="content">
			<form id="updateForm" name="houseApp.updateForm" action="houseApp/update.do" AUTOCOMPLETE="off">
				...
				<table id="appInfo">
					...
					<tr>
						...
						<td><input id="belongChannel" name="houseApp.belongChannel" value="${bean.belongChannel}" type="text" class="easyui-combobox" data-options="required:true,validType:['length[0,50]']" editable="false"/></td>
			...
			<img src="img/deleteItem2.png"  onclick="rmObj('dealingsContact_dealingsContactsIndex');" />
				...
				<table id="houseInfo">
					<tr>
						...
						class="textbox easyui-validatebox" value="${houseInfo.houseCertificateNumber}"/></td>
						<td>土地证号:</td>
					...
					<tr>
						...
						class="easyui-combobox" value="${houseInfo.houseExistProvince}" editable="false" style="width:140px;"/>省
						<input id="houseExistCity" name="houseInfo.houseExistCity" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox" value="${houseInfo.houseExistCity}" editable="false" style="width:140px;"/>市
						<input id="houseExistCounty" name="houseInfo.houseExistCounty" type="text" data-options="required:true,validType:['length[0,10]']" 
						class="easyui-combobox" value="${houseInfo.houseExistCounty}" editable="false" style="width:130px;"/>区县
						<input id="houseExistAddress" name="houseInfo.houseExistAddress" type="text" data-options="required:true,validType:['length[0,100]']" 
						class="textbox easyui-validatebox" value="${houseInfo.houseExistAddress}" editable="false"/></td>
					</tr>
				</table>
				...
				<tr>
					...
					class="textbox easyui-validatebox" value="${bean.assessComp}"/></td>
				</tr>
				<tr>
					<td>评估描述：</td>
					<td colspan="5">
						<textarea id="assessRemark" name="houseApp.assessRemark" value="${bean.assessRemark}" class="textbox easyui-validatebox" data-options="required:true,validType:['length[0,500]']" 
				...
				<table>	
					...
					<tr>
						<td colspan=3">
							...
							<input type="button" value="前端拒贷" class="btn" onclick="javascript:artOpenPage('前端拒贷','house/app/frontRefusePopup.do?id=${bean.id}');" title="拒贷"/>&nbsp;&nbsp;
...
$(document).ready(function (){
	...
	$('#belongChannel').combobox({
		...
		onChange: function(newValue, oldValue){             
			...
			var producturl = "product/hedao/listjason.do?type=5&state=1&belongChannel=" + encodeURI(newValue);
			$('#product').combobox('reload',producturl); 
		}
	});
	//填充select数据 产品
	var belongChannel = $('#belongChannel').combobox('getValue');
	var producturl = "product/hedao/listjason.do?type=5&state=1&belongChannel=" + encodeURI(belongChannel);
	$("#product").combobox("clear");
	$('#product').combobox({
		url:producturl,
		valueField:'name', 
    ...
    var province = $('#firmProvince').combobox('getValue');
		...
		//填充select数据 房屋所在省份
	    var provinceurl = "sys/datadictionary/listjason.do?keyName=province";
			$("#houseExistProvince").combobox("clear");
			$('#houseExistProvince').combobox({
				url: provinceurl,
				valueField: 'keyProp',
				textField: 'keyValue',
				onChange: function(newValue, oldValue){
	            $('#houseExistCity').combobox('clear');
	            $('#houseExistCounty').combobox('clear');
	            var cityurl = "sys/datadictionary/listjason.do?keyName=city&parentKeyProp=" + encodeURI(newValue);
	            $('#houseExistCity').combobox('reload',cityurl); 
	        }
			});
	    //填充select数据 房屋所在地市
	    var province = $('#houseExistProvince').combobox('getValue');
			var cityurl = "sys/datadictionary/listjason.do?keyName=city&parentKeyProp=" + encodeURI(province);
			$("#houseExistCity").combobox("clear");
			$('#houseExistCity').combobox({
				url: cityurl,
				valueField: 'keyProp',
				textField: 'keyValue',
				onChange: function(newValue, oldValue){
	            $('#houseExistCounty').combobox('clear');
	            var countyurl = "sys/datadictionary/listjason.do?keyName=county&parentKeyProp=" + encodeURI(newValue);
	            $('#houseExistCounty').combobox('reload',countyurl); 
	        }
			});
	    //填充select数据 房屋所在地区县
	    var city = $('#houseExistCity').combobox('getValue');
			var countyurl = "sys/datadictionary/listjason.do?keyName=county&parentKeyProp=" + encodeURI(city);
			$("#houseExistCounty").combobox("clear");
			$('#houseExistCounty').combobox({url: countyurl, valueField: 'keyProp',textField: 'keyValue'});
