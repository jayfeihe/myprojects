			...
			<table id="table" class="datatable" summary="list of members in EE Studay">
				...
				<c:forEach items="${ pm.data}" var="data" varStatus="status">
					<tr>
						...
						<td><a href="<c:if test="${stateType eq 'offTask' }">house/query/read.do?id=${data.id }</c:if>house/app/read.do?id=${data.id}" target="_blank">${data.appId}</a></td>
						<td>${data.name}</td>
						...
						<td>${data.product}</td>
						<td>${data.houseDecisionOperator}</td>
						<td>
							<c:choose>
								<c:when test="${data.houseDecisionCode=='00'}">退回</c:when>
								<c:when test="${data.houseDecisionCode=='02'}">拟通过</c:when>
								<c:when test="${data.houseDecisionCode=='03'}">拟拒贷</c:when>
							</c:choose>	
						</td>
						<td>
							<c:choose>
...
function updateData(data_id,name) {
    var url="<%=basePath%>house/approval/update.do?id="+data_id;
    addTab(name, url);
    return null;
}  
