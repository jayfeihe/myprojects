...
<%--		</span>--%>
		<span style="padding-left: 20px; color: red;">
			...
			<c:if test="${fn:contains(houseVerify.product,'精英贷') and not empty bean.gradeRemind}">&nbsp;&nbsp;|&nbsp;&nbsp;${bean.gradeRemind}</c:if>
		</span>
		<span style="padding-left: 20px; color: red;">
			授信额度：15万元
		</span>
...
<div data-options="region:'east',split:true,border:false" style="width:800px;">
	<div class="easyui-tabs" id="verify_tabs" style="height:730px;" data-options="fit:true,onLoad:tabsReadOnly">
		<div title="风险提示" data-options="href:'${basePath}house/rule/read.do'" style="width: 100%;padding:10px"></div>
...
<%--								class="easyui-combobox"  value="${bean.period}" editable="false" style="width:152px;" disabled="disabled"/></td>--%>
			 	...
			 	<tr><td>
			 		<table id="verifyInfo">
			 			<tr>
							<td align="right">借款金额：</td>
							<td><input id="amount" value="${houseVerify.amount/10000}" type="text"
...
<%--							<td ><input id="period" value="${houseVerify.period}" type="text" data-options="validType:['length[0,10]']" --%>
...
<%--							</td>--%>
							...
							<td>
								...
								data-options="validType:['length[0,50]']" editable="false" value="${houseVerify.belongChannel}" disabled="disabled"/>
							</td>
							<td align="right">产品：</td>
							<td>
								<input id="product" value="${houseVerify.product}" type="text"  data-options="validType:['length[0,50]']" 
								class="easyui-combobox" editable="false" style="width:152px;" disabled="disabled"/>
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td ><input value="${houseVerify.operator}" type="text"
										data-options="validType:['length[0,30]']"
										class="textbox easyui-validatebox" style="width:148px;" disabled="disabled"/>
								<input type="hidden" name="houseVerifyOperator" value="${houseVerify.operator}" /></td>
							<td colspan="4">&nbsp;</td>
						</tr>
						<tr>
							<td align="right">建议：</td>
							<td><select id="decision" value="${houseVerify.decision}"
									data-options="validType:['length[0,50]']" 
									class="easyui-combobox" editable="false" style="width:152px;" disabled="disabled">
									<option value="">请选择</option>
									<option value="02" <c:if test="${'02' eq houseVerify.decision}">selected</c:if>>拟通过</option>
									<option value="03" <c:if test="${'03' eq houseVerify.decision}">selected</c:if>>拟拒贷</option>
								</select>
							</td>
							<c:if test="${'03' eq houseVerify.decision}">
								<td align="right" colspan="4">
									违例码：
									<input id="decisionCode1" 
										value="${houseVerify.decisionCode1}" type="text" 
										data-options="validType:['length[0,50]']" 
										class="easyui-combobox" editable="false" disabled="disabled" style="width:152px;"/>
									<input id="decisionCode2" 
										value="${houseVerify.decisionCode2}" type="text" 
										data-options="validType:['length[0,50]']" 
										class="easyui-combobox" editable="false" disabled="disabled" style="width:152px;"/>
								</td>
							</c:if>
						...
						<tr>
							...
							<td colspan="5"><textarea value="${houseVerify.remarks}" class="textbox easyui-validatebox" data-options="validType:['length[0,500]']" 
								style="resize: none;width:625px;height:50px!important;background-color: #F0F0F0;" disabled="disabled">${houseVerify.remarks}</textarea></td>
						</tr>
					</table>
				</td></tr>
					...
					<td colspan="9">
						<div style="font-size: 14px;" >
							<strong>审批决策</strong>
								...
								<input type="hidden" name="decision.id" value="${houseApproval.id}" />
								<input type="hidden" name="decision.appId" value="${bean.appId}" />
								<input type="hidden" name="decision.type" value="2" />
								<input type="hidden" id="state" name="decision.state" value="${houseApproval.state}" />
							<hr width="100%" size=2 color="#D3D3D3" noshade>
						</div>
					</td>
				</tr>
				<tr><td>
			 		<table id="approvalInfo">
			 			<tr>
							<td align="right">借款金额：</td>
							<td><div id ="amountDiv">
								<input id="amount" name="decision.amount" value="${houseApproval.amount/10000}" type="text"
...
<%--							<td align="light">借款期限:</td>--%>
<%--							<td ><input id="period" name="decision.period" <c:if test="${0 != houseApproval.period}">value='${houseApproval.period}'</c:if> type="text" data-options="required:true,validType:['length[0,10]']" --%>
...
<%--							</td>--%>
							...
							<td>
								...
								data-options="required:true,validType:['length[0,50]']" editable="false" value="${houseApproval.belongChannel}"/>
							</td>
							<td align="right">产品：</td>
							<td>
								<input id="product" name="decision.product" value="${houseApproval.product }"
...
<%--							测试码--%>
							...
							<td style="display: none;" id="selectCode" class="JYDTestScheme">
								<input id="secondTestCode" name="decision.secondTestCode" value="${houseApproval.secondTestCode}"
									data-options="" class="easyui-combobox" editable="false" style="width:120px;"/>
								<input type="hidden" id="firstTestCode" name="decision.firstTestCode" value="${houseApproval.firstTestCode}"/>
								<input type="hidden" id="secondTestValue" name="decision.secondTestValue" value="${houseApproval.secondTestValue}"/>
							</td>
						</tr>
						<tr>
							...
							<td>
								<input id="htje" type="text" data-options="min:0,precision:2" class="textbox easyui-numberbox" value="${houseApproval.contractAmount}" style="width:152px;" onchange="updateJkjeYhkje();"/> 元
							</td>
						</tr>
						...
						<tr>
							...
							<td><select id="decision" name="decision.decision" value="${houseApproval.decision}"
									...
									class="easyui-combobox" editable="false" style="width:152px;">
									<option value="">请选择</option>
									<option value="00" <c:if test="${'00' eq houseApproval.decision}">selected</c:if>>退回</option>
									<option value="01" <c:if test="${'01' eq houseApproval.decision}">selected</c:if>>通过</option>
									<option value="04" <c:if test="${'04' eq houseApproval.decision}">selected</c:if>>特殊审批</option>
									<option value="05" <c:if test="${'05' eq houseApproval.decision}">selected</c:if>>拒贷</option>
									<option value="06" <c:if test="${'06' eq houseApproval.decision}">selected</c:if>>推送线上</option>
								</select>
							</td>
							<td align="right" colspan="4">
								<div id ="noPassReasonDiv">
									退回原因：
									<textarea id="returnMsg" name="decision.returnMsg" value="${houseApproval.returnMsg}" class="textbox easyui-validatebox"
									data-options="required:true,validType:['length[0,500]']" 
									style="resize: none;width:325px;height:50px!important;vertical-align:middle;">${houseApproval.returnMsg}</textarea>
								</div>
								<div id ="operatorDiv">
									高级处理人：
								...
								<div id ="decisionCodeDiv1">
									...
									<input id="decisionCode3" name="decision.decisionCode3" 
										value="${houseApproval.decisionCode3}" 
										type="text" data-options="validType:['length[0,50]']" 
										class="easyui-combobox" editable="false" style="width:152px;"/>
									<input id="decisionCode4" name="decision.decisionCode4" 
										value="${houseApproval.decisionCode4}" 
										type="text" data-options="validType:['length[0,50]']" 
										class="easyui-combobox" editable="false" style="width:152px;"/>
								</div>
							</td>
						...
						<tr>
							<td align="right">备注：</td>
								...
								value="${houseApproval.remarks}" class="textbox easyui-validatebox" data-options="validType:['length[0,100]']"
								style="resize: none;width:625px;height:50px!important;">${houseApproval.remarks}</textarea></td>
						</tr>
						<tr>
							<td>
								<td colspan="6">
...
function updateFunction(buttonType) {
	...
	$.ajax({
		type : "POST",
		url : "<%=basePath%>house/approval/save.do",
...
function updateYhkjeHtje(){
	if($('#approvalInfo').find('#amount').val() != '' && $('#approvalInfo').find('#product').combobox('getValue') != ''){
		updateJe("<%=basePath%>" + "house/verify/getJe.do", "yhkje=0.00&htje=0.00&jkje=" + $('#approvalInfo').find('#amount').val() + "&productName=" + $('#approvalInfo').find('#product').combobox('getValue'), "jkje");
	}
}
...
function updateJkjeHtje(){
	if($('#approvalInfo').find('#yhkje2').val() != '' && $('#approvalInfo').find('#product').combobox('getValue') != ''){
		updateJe("<%=basePath%>" + "house/verify/getJe.do", "jkje=0.00&htje=0.00&yhkje=" + $('#approvalInfo').find('#yhkje2').val() + "&productName=" + $('#approvalInfo').find('#product').combobox('getValue'), "yhkje");
	}
}
...
function updateJkjeYhkje(){
	if($('#approvalInfo').find('#htje').val() != '' && $('#approvalInfo').find('#product').combobox('getValue') != ''){
		updateJe("<%=basePath%>" + "house/verify/getJe.do", "jkje=0.00&yhkje=0.00&htje=" + $('#approvalInfo').find('#htje').val() + "&productName=" + $('#approvalInfo').find('#product').combobox('getValue'), "htje");
	}
}
...
$(document).ready(function (){
	...
	if('${houseApproval.decision}' == '00'){
		...
		$('#feedbackSellDescribeDiv2').hide();//隐藏div  
		toggleValidate('#feedbackSellDescribeDiv2',false);
	}else if('${houseApproval.decision}' == '01'){
		$('#noPassReasonDiv').hide();//隐藏div    
		toggleValidate('#noPassReasonDiv',false);
		$('#operatorDiv').hide();//隐藏div  
		toggleValidate('#operatorDiv',false);
		...
		$('#feedbackSellDescribeDiv2').hide();//隐藏div  
		toggleValidate('#feedbackSellDescribeDiv2',false);
	}else if('${houseApproval.decision}' == '05'){
		$('#noPassReasonDiv').hide();//隐藏div    
		toggleValidate('#noPassReasonDiv',false);
		$('#operatorDiv').hide();//隐藏div  
		toggleValidate('#operatorDiv',false);
		...
		$('#feedbackSellDescribeDiv2').show();//反馈信息div  
		toggleValidate('#feedbackSellDescribeDiv2',true);
	}else if('${houseApproval.decision}' == '04'){
		$('#noPassReasonDiv').hide();//隐藏div  退回原因  
		toggleValidate('#noPassReasonDiv',false);
		$('#operatorDiv').show();//显示div  高级处理人
		toggleValidate('#operatorDiv',true);
	...
	var userurl=encodeURI("sys/user/listUserByOrgAndRole.do?orgId=${login_org.orgId}&roleNames=房贷高级审批员");
...
*/
	...
	$("#verifyInfo").find('#belongChannel').combobox({
		...
		onChange: function(newValue, oldValue){             
			...
			var producturl1 = "product/hedao/listjason.do?type=5&flag=1&belongChannel=" + encodeURI(newValue);
			$("#verifyInfo").find('#product').combobox('reload',producturl1); 
		}
	});
	//填充select数据 产品
	var belongChannel1 = $("#verifyInfo").find('#belongChannel').combobox('getValue');
	var producturl1 = "product/hedao/listjason.do?type=5&flag=1&belongChannel=" + encodeURI(belongChannel1);
	$("#verifyInfo").find("#product").combobox("clear");
	$("#verifyInfo").find('#product').combobox({
		...
		onChange: function(productVal){
			if($('#verifyInfo').find('#amount').val() != '')
				updateJe("<%=basePath%>" + "house/verify/getYhkje.do", "fkje=" + $('#verifyInfo').find('#amount').val() + "&productName=" + productVal, "yhkje");
		}	
	});
	...
	$("#approvalInfo").find('#belongChannel').combobox({
		...
		onChange: function(newValue, oldValue){             
			...
			var producturl2 = "product/hedao/listjason.do?type=5&state=1&flag=1&belongChannel=" + encodeURI(newValue);
	...
	var producturl2 = "product/hedao/listjason.do?type=5&state=1&flag=1&belongChannel=" + encodeURI(belongChannel2);
	$("#approvalInfo").find("#product").combobox("clear");
