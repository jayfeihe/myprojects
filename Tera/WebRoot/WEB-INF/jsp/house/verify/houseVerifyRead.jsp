...
<%--								class="easyui-combobox"  value="${bean.period}" editable="false" style="width:152px;" disabled="disabled"/></td>--%>
			 	...
			 	<tr>
					<td colspan="9">
						<div style="font-size: 14px;" >
							<strong>审核</strong>
								<input type="hidden" name="decision.id" value="${houseVerify.id}" />
			 	...
			 	<tr><td>
			 		<table id="verifyInfo">
			 			<tr>
							<td align="right">借款金额:</td>
							<td><div id ="amountDiv">
							<input id="amount" name="decision.amount" value="${houseVerify.amount/10000}" type="text"
...
<%--							<td ><input id="period" name="decision.period" value="${houseVerify.period}" type="text" data-options="required:true,validType:['length[0,10]']" --%>
...
<%--							</td>--%>
							<td align="right">渠道:
								...
								data-options="validType:['length[0,50]']" editable="false" value="${houseVerify.belongChannel}"/>
							</td>
							<td align="right">产品：
								<input id="product" name="decision.product" <c:choose><c:when test="${null == houseVerify.product || '' eq houseVerify.product}">value='${bean.product }'</c:when><c:otherwise>value='${houseVerify.product }'</c:otherwise></c:choose> type="text"  data-options="required:true,validType:['length[0,50]']" 
						...
						<tr>
							...
							<td>
								<input id="decision" name="decision.decision" value="${houseVerify.decision}" type="text"  data-options="required:true,validType:['length[0,50]']" 
							...
							<td align="center" colspan="4" valign="top">
								<div id="noPassReasonDiv" >
									...
									<input id="decisionCode5" name="decision.decisionCode5" 
										value="${houseVerify.decisionCode5}" type="text" 
										data-options="validType:['length[0,50]']" 
										class="easyui-combobox" editable="false" style="width:152px;background-color: #F0F0F0;" readonly="readonly"/>
									<input id="decisionCode6" name="decision.decisionCode6" 
										value="${houseVerify.decisionCode6}" type="text" 
										data-options="validType:['length[0,50]']" 
										class="easyui-combobox" editable="false" style="width:152px;background-color: #F0F0F0;" readonly="readonly"/>
									
<%--								<input type="button" class="btn" value="补充资料" onclick="addObj('commonContacts');"/>--%>
<%-- 									<input type="button" class="btn" value="补充资料" onclick="javascript:artOpenPage('影像资料','house/verify/addImgDataPopup.do?appId=${bean.appId}')"> --%>
									<br>
									退回原因:
									<textarea id="returnMsg" name="decision.returnMsg" value="${houseVerify.returnMsg}" class="textbox easyui-validatebox"
									data-options="required:true,validType:['length[0,500]']" 
									style="resize: none;width:390px;height:50px!important;vertical-align:middle;background-color: #F0F0F0;" readonly="readonly">${houseVerify.returnMsg}</textarea>
								</div>
...
<%--						</tr>--%>
						...
						<tr>
							<td>备注：</td>
							<td colspan="5">
								<textarea id="remarks" name="decision.remarks" value="${houseVerify.remarks}"
								class="textbox easyui-validatebox" data-options="validType:['length[0,100]']"
								style="resize: none;width:625px;height:50px!important;background-color: #F0F0F0;" readonly="readonly">${houseVerify.remarks}</textarea>
							</td>
						</tr>
						<tr>
							<td colspan="6">
...
// 	$.ajax({
// 		type : "POST",
<%-- 		url : "<%=basePath%>house/antifraud/update.do", --%>
// 		data : params+"&buttonType="+buttonType,
// 		success : function(data) {
// 			//关闭遮罩，弹出消息框
// 			window.parent.closeMask();
...
function updateFunction(buttonType) {
	...
	$.ajax({
		type : "POST",
		url : "<%=basePath%>house/verify/save.do",
		data : params+"&buttonType="+buttonType,
		success : function(data) {
			//关闭遮罩，弹出消息框
			window.parent.closeMask();
...
function updateYhkje(){
	if($('#verifyInfo').find('#amount').val() != '' && $('#verifyInfo').find('#product').combobox('getValue') != ''){
		updateJe("<%=basePath%>" + "house/verify/getYhkje.do", "fkje=" + $('#verifyInfo').find('#amount').val() + "&productName=" + $('#verifyInfo').find('#product').combobox('getValue'), "yhkje");
	}
}
...
function updateFkje(){
	if($('#verifyInfo').find('#yhkje').val() != '' && $('#verifyInfo').find('#product').combobox('getValue') != ''){
		updateJe("<%=basePath%>" + "house/verify/getFkje.do", "yhkje=" + $('#verifyInfo').find('#yhkje').val() + "&productName=" + $('#verifyInfo').find('#product').combobox('getValue'), "fkje");
	}
}
...
//填充select数据 解除反欺诈结果
	$("#houseAntifraud").find("#result").combobox("clear");
	$("#houseAntifraud").find('#result').combobox({
		valueField:'keyProp', 
		textField:'keyValue',
