...
<%--							</td>--%>
							<td align="right">渠道:
								...
								data-options="required:true,validType:['length[0,50]']" editable="false" value="${houseVerify.belongChannel}"/>
							</td>
							<td align="right">产品：
								<input id="product" name="decision.product" value="${houseVerify.product }" type="text"  data-options="required:true,validType:['length[0,50]']" 
								class="easyui-combobox" editable="false" style="width:152px;"/>
							</td>
						</tr>
						<tr>
...
<%-- 									<input type="button" class="btn" value="补充资料" onclick="javascript:artOpenPage('影像资料','house/verify/addImgDataPopup.do?appId=${bean.appId}')"> --%>
									<br>
									退回原因:
									<textarea id="returnMsg" name="decision.returnMsg" value="${houseVerify.returnMsg}" class="textbox "
									data-options="required:true,validType:['length[0,500]']" 
									style="resize: none;width:390px;height:50px!important;vertical-align:middle;">${houseVerify.returnMsg}</textarea>
								</div>
...
<%--						</tr>--%>
						...
						<tr>
							...
							<td colspan="5">
								...
								style="resize: none;width:625px;height:50px!important;">${houseVerify.remarks}</textarea>
							</td>
						</tr>
						<tr>
							<td colspan="6">
...
$(document).ready(function (){
	...
	}else if('${houseVerify.decision}' == '02'){
		$('#noPassReasonDiv').hide();//div 
		toggleValidate('#noPassReasonDiv',false);
		$("#imageClassListDiv").hide();
		$("#imageClassSelectDiv").hide();
...
//填充select数据 借款期限
	...
	$("#verifyInfo").find('#belongChannel').combobox({
		...
		onChange: function(newValue, oldValue){             
			...
			var producturl1 = "product/hedao/listjason.do?type=5&state=1&flag=1&belongChannel=" + encodeURI(newValue);
			$("#verifyInfo").find('#product').combobox('reload',producturl1); 
		}
	});
	//填充select数据 产品
	var belongChannel1 = $("#verifyInfo").find('#belongChannel').combobox('getValue');
	var producturl1 = "product/hedao/listjason.do?type=5&state=1&flag=1&belongChannel=" + encodeURI(belongChannel1);
	$("#verifyInfo").find("#product").combobox("clear");
