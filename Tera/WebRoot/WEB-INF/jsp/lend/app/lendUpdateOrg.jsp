...
<div id="main">
	<div id="part1" class="part">
		<p class="title"><a href="javascript:void(0);">企业客户录入申请</a></p>
		<div class="content">
			<form id="updateForm" >
			<input type="hidden" name="customerType" value="02" <c:if test="${bean!=null}">disabled="disabled"</c:if> />
			...
			<table>
				...
				<tr>
					<td>证件类型:</td>
					<td><input <c:if test="${bean!=null}">value="${bean.idType}" "</c:if> class="easyui-combobox" name="idType" id="idType" editable="false" data-options="required:true,validType:['length[0,18]']"/></td>
					<td>证件编号:</td>
					<td><input <c:if test="${bean!=null}">value="${bean.idNo}" "</c:if> id="idNo" onblur="getCustomer();" name="idNo" type="text" data-options="required:true,validType:['length[0,18]','idNo']" class="textbox easyui-validatebox" /></td>
					<td>机构全称:</td>
					<td><input <c:if test="${bean!=null}">value="${bean.name}"</c:if>  id="name" name="name" type="text" data-options="required:true,validType:['length[0,50]']" class="textbox easyui-validatebox" /></td>
				</tr>
				<tr>
					<td>电话:</td>
					...
					<td>财务负责人:</td>
					<td><input <c:if test="${bean!=null}">value="${bean.cfoName}"</c:if> id="cfoName" name="cfoName" type="text" data-options="validType:['length[0,50]']" class="textbox easyui-validatebox" /></td>
					<td>手机号:</td>
					<td><input <c:if test="${bean!=null}">value="${bean.mobile}"</c:if> id="mobile" name="mobile" type="text" data-options="validType:['mobile']" class="textbox easyui-validatebox" /></td>
				</tr>
				<tr>
					<td>申请类型:</td>
					<td><input <c:if test="${bean!=null}">value="${bean.appType}"</c:if> id="appType" name="appType"  data-options="required:true,validType:['length[0,18]']" class="easyui-combobox" editable="false" /></td>
					<td>出借金额:</td>
					<td>
						 <input class="easyui-numberspinner" id="amount" name="amount"
						 data-options="min:50000,increment:10000,precision:2,groupSeparator:','
					...
					<td>产品:</td>
					<td><input <c:if test="${bean!=null}">value="${bean.product}"</c:if> id="product" name="product" data-options="required:true,validType:['length[0,50]']" class="easyui-combobox" editable="false" /></td>
				</tr>
				<tr>
					<td>服务截至日期:</td>
					<td><input <c:if test="${bean!=null}">value="${bean.serviceEndDate}"</c:if> id="serviceEndDate" name="serviceEndDate" type="text" data-options="required:true" editable="false" class="textbox easyui-datebox" /></td>
					<td>营销人员:</td>
                    ...
                    class="easyui-combobox" name="orgId" id="orgId" 
                    readonly="readonly" data-options="validType:['length[0,18]']"/></td>
				</tr>
<!-- 				<tr> -->
<!-- 					<td>出借账户名:</td> -->
<%-- 					<td><input <c:if test="${bean!=null}">value="${bean.lendAccName}"</c:if> id="lendAccName" name="lendAccName" type="text" data-options="validType:['length[0,50]']" class="textbox easyui-validatebox" /></td> --%>
<!-- 					<td>出借开户银行:</td> -->
<%-- 					<td><input <c:if test="${bean!=null}">value="${bean.lendAccBank}"</c:if> id="lendAccBank" name="lendAccBank" type="text" data-options="validType:['length[0,50]']" class="textbox easyui-validatebox" /></td> --%>
<!-- 					<td>出借银行账号:</td> -->
<%-- 					<td><input <c:if test="${bean!=null}">value="${bean.lendAccount}"</c:if> id="lendAccount" name="lendAccount" type="text" data-options="validType:['length[0,50]','NumLetters']" class="textbox easyui-validatebox" /></td> --%>
<!-- 				</tr> -->
				<tr>
					<td>开户行省:</td>
					<td>
						<input id="bankProvince" name="bankProvince" type="text" class="textbox easyui-combobox" data-options="required:true" value="${bean.bankProvince}" editable="false"/>
					</td>
					<td>开户行市:</td>
					<td>
						<input id="bankCity" name="bankCity" type="text" class="textbox easyui-combobox" data-options="required:true" value="${bean.bankCity}" editable="false"/>
					</td>
					<td>POS关联手机号:</td>
				...
				<tr>
					<td>撮合类型:</td>
					<td><input <c:if test="${bean!=null}">value="${bean.matchType}"</c:if> id="matchType" name="matchType"  data-options="required:true,validType:['length[0,50]']" class="easyui-combobox" editable="false" /></td>
					<td>合同编号:</td>
...
function getCustomer(){
	...
	if(zjNo&& zjNo!="" && zjNoType &&zjNoType!=""&&(zjNo!=lsZjNo||zjNoType!=lsNoType)){
		...
		$.ajax({
			...
			data : encodeURI("customerType=02&zjNumber=" + zjNo+"&idType="+zjNoType),
			dataType : "json",
			success : function(data) {
				if(data.bean){
