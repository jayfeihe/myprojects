...
<body>
	...
	<div id="main">
		<div id="part1" class="part">
			<p class="title">
				<a href="javascript:void(0);">总部审核</a>
			</p>
			<form id="updateForm">
						...
						<table width="100%">
							<tbody>
								...
								<tr>
									...
									<td>贷记卡违约值:</td>
									<td><select id="selfCreditCardDvalue" editable="false"
								...
								<tr>
									<td><input type="radio" id="selfCreditMark" name="selfCreditMark"
										 value='0'
										<c:if test="${bean.selfCreditMark=='0'}">checked='checked'</c:if> />正常
										<!-- 							<br/> --> <input type="radio" id="selfCreditMark"
										name="selfCreditMark" value="1"
								...
								<tr>
									<td>配偶信用正常标志:</td>
									<td>配偶贷记卡违约值:</td>
									<td><select id="spouseCreditCardDvalue" editable="false"
								...
								<tr>
									<td><input type="radio" id="spouseCreditMark" name="spouseCreditMark"
										value='0'
										<c:if test="${bean.spouseCreditMark=='0'}">checked='checked'</c:if> />正常
										<!-- 							<br/> --> <input type="radio" id="spouseCreditMark"
										name="spouseCreditMark" value="1"
								...
								<tr>
									<td><input type="radio" id="entLoanMark" name="entLoanMark"
										value='0'
										<c:if test="${bean.entLoanMark=='0'}">checked='checked'</c:if> />正常
										<!-- 							<br/> --> <input type="radio" id="entLoanMark"
										name="entLoanMark" value="1"
			...
			<div id="approvalExamine">
				<table width="100%">
					<tr>
						<td id="imgDiv">
						<jsp:include page="/img/imgSlidePath.do?appId=${appId}" />
						</td>
					</tr>
				</table>
				<table width="100%">
					...
					<tr>
						...
						<td>担保条件:</td>
						<td colspan="3"><textarea id="guaranteeCondition"
								name="guaranteeCondition"
								data-options="validType:['length[0,500]']"
								style="width: 100%;height:100px !important;" class="textbox easyui-validatebox">${bean.guaranteeCondition}</textarea>
						</td>
					</tr>
					<tr>
						...
						<td><input type="button" value="通过" class="btn"
							onclick="bmmupdateFunction(1)" /> <input type="button"
							value="特殊审核" class="btn" onclick="bmmupdateFunction(3)" /></td>
					</tr>
					<tr>
						<td></td>
						<td><input type="button" value="不通过" class="btn"
							onclick="bmmupdateFunction(0)" /></td>
						<td colspan="4">不通过原因：<input id="auditText_btg" type="text"
							class="textbox easyui-validatebox" />
						</td>
					</tr>
					<tr>
						<td></td>
						<td><input type="button" value="拒件" class="btn"
							onclick="bmmupdateFunction(2)" /></td>
						<td  colspan="4">拒件原因：<input id="auditText_jujian" type="text"
							class="textbox easyui-validatebox" />
						</td>
					</tr>
					<tr>
						<td><input type="button" value="保存" class="btn"
							onclick="updateFunction()" /> <input type="button" value="返回"
							class="btn" onclick="javascript:back()" /></td>
					</tr>
					<tr>
						<td></td>
					</tr>
...
<form id="fileSmt" action="file/upload/zipupload.do"  enctype="multipart/form-data" style="display: none;" >
	<input type="hidden" id="fileappId" name="appId" <c:if test="${bean!=null}">value="${appId}"</c:if>/>
</form>
...
function bmmupdateFunction(val) {
	...
	if(val==0){
        ...
        var a = $(":radio[name='selfCreditMark']:checked").val();
        var b = $(":radio[name='spouseCreditMark']:checked").val();
        var c = $(":radio[name='entLoanMark']:checked").val();
        var d = $(":radio[name='courtCompany']:checked").val();
        var e = $(":radio[name='courtLegal']:checked").val();
        var f = $(":radio[name='courtGuarantee']:checked").val();
        var g = $(":radio[name='industryNetwork']:checked").val();
        var h = $(":radio[name='orgNetwork']:checked").val();
        var i = $(":radio[name='telLegal']:checked").val();
        var j = $(":radio[name='telAgent']:checked").val();
        var k = $(":radio[name='telCfo']:checked").val();
        var l = $(":radio[name='telContract']:checked").val();
		var arr=[a,b,c,d,e,f,g,h,i,j,k,l];
		for(ii=0;ii<11;ii++){
			if(arr[ii]==null || arr[ii]==""){
				//alert("第"+(ii+1)+"个没有选择，值是：+"arr[ii]"+。");
				$.messager.confirm('消息', "请选择第"+(ii+1)+"个单选信息框");
				return;
			}
		}
	}
...
function updateFunction() {
	...
	$("#approvalExamine").find('textarea').each(function(){
		$(this).val($.trim($(this).val()));
	});
	...
	$.ajax({
		type : "POST",
		url : "<%=basePath%>" + "loan/approval/save.do",
