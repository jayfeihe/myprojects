...
<%--													<fmt:formatNumber type="number" value="${data.lendAmount } " maxFractionDigits="2"/>--%>
						...
						<br/>
				<input type="button" class="btn" value="通过" onclick="javascript:passManualMatchVerify('updateForm')"/><br/><br/>
				<input type="button" class="btn" value="不通过" onclick="javascript:denyManualMatchVerify()"/>
				<label>不通过原因：</label>
...
//审批不通过执行的方法
function denyManualMatchVerify() {

	var loan2matchId_matchresult = document.getElementById("loan2matchId").value;
	var denyReason = document.getElementById("denyReason").value;
	if(denyReason == ""){
		$.messager.confirm('消息', "请填写不通过原因", function(ok) {
			if (ok) {
				denyReason.value="";
				denyReason.focus();
			}
	...
	//var formAction = "lend/manualmatchverify/denyManualMatchVerify.do";
	var formAction = "loan/manualverify/denyManualMatchVerify.do";
	openMask();
	
 $.ajax({
		type : "POST",
		url : formAction + "?loan2matchId_matchresult=" + loan2matchId_matchresult.trim() + "&&denyReason=" + denyReason.trim(),
		success : function(data) {
