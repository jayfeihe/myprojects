			...
			<table id="table" class="datatable" summary="list of members in EE Studay">
				...
				<c:forEach items="${ pm.data}" var="data" varStatus="status">
					<tr>
						...
						<td>
							<c:choose>
								<c:when test="${data.state eq '1'}">未支付</c:when>
								<%-- <c:when test="${data.state eq '2'}">已发盘</c:when>
								<c:when test="${data.state eq '3'}">发盘成功</c:when>
								<c:when test="${data.state eq '4'}">发盘失败</c:when> --%>
								<c:when test="${data.state eq '5'}">支付成功</c:when>
								<c:when test="${data.state eq '6'}">支付失败</c:when>
								<c:when test="${data.state eq '9'}">未确认</c:when>
							</c:choose>
						</td>
...
function showData(data_id) {
	...
	var _url = "<%=basePath%>" + "thirdPayment/update.do?id=" + data_id;
