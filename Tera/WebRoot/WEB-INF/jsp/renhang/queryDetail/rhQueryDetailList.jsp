...
<td>${data.appId}</td>
<td>${data.queryId}</td>
<td>${data.queryDate}</td>
<td>${data.queryOperator}</td>
...
function updateData(data_id) {
	window.location = "<%=basePath%>" + "renhang/queryDetail/update.do?id=" + data_id;
	return;
}
...
function deleteData(data_id) {
	$.messager.confirm('消息', '您确认要删除？', function(ok){
		...
		if (ok){
			$.ajax({
				url : "<%=basePath%>" + "renhang/queryDetail/delete.do",
...
//添加
function add(){
	window.location = "<%=basePath%>" + "renhang/queryDetail/update.do";
	return;
}
