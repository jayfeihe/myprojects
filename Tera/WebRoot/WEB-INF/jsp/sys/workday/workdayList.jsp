...
<td>
	<c:if test="${data.work == '0'}" >
	休息
	</c:if>
	<c:if test="${data.work == '1'}" >
	上班
	</c:if>
</td>
<td>${data.remark}</td>
...
function updateData(data_id) {
	window.location = "<%=basePath%>" + "sys/workday/update.do?id=" + data_id;
...
function deleteData(data_id) {
	...
	$.messager.confirm('消息', '您确认要删除？', function(ok){
		...
		if (ok){
			$.ajax({
				url : "<%=basePath%>" + "sys/workday/delete.do",
...
//添加
function add(){
	window.location = "<%=basePath%>" + "sys/workday/update.do";
