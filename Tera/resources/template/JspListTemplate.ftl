...
function updateData(data_id) {
	window.location = "<%=basePath%>" + "${jspPath}/update.do?${primaryProp}=" + data_id;
	return;
}
...
function deleteData(data_id) {
	$.messager.confirm('消息', '您确认要删除？', function(ok){
		...
		if (ok){
			$.ajax({
				url : "<%=basePath%>" + "${jspPath}/delete.do",
				// 防止浏览器缓存，在URL后加时间戳 "&timestamp=" + (new Date()).getTime()
				data : encodeURI("${primaryProp}=" + data_id + "&timestamp=" + (new Date()).getTime()),
				async : false,// 同步提交
...
//添加
function add(){
	window.location = "<%=basePath%>" + "${jspPath}/update.do";
	return;
}
