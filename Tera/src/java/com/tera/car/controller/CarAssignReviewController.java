 ...
 */
@Controller
@RequestMapping("/car/assign/review")
public class CarAssignReviewController  extends BaseController{

	/**
	 * 日志
	 */
	private final static Logger log = Logger.getLogger(CarAssignReviewController.class);
	...
	public String assignReviewQuery(HttpServletRequest request, ModelMap map) throws Exception {
		...
		return "car/assign/review/assignQuery";
	}
	...
	public String assignReviewList(CarQBean qBean,HttpServletRequest request, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		...
		map.put("pm", pm);
		log.info(thisMethodName+":end");
		return "car/assign/review/assignList";
	}
