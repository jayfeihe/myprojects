...
import com.tera.car.model.form.ReviewFBean;
...
public class CarReviewController {
	 ...
	 * 日志
	 */
	private final static Logger log = Logger.getLogger(CarReviewController.class);
	...
	public String carReviewQuery(HttpServletRequest request, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		log.info(thisMethodName+":end");
		return "car/review/carReviewQuery";
	}
	...
	public String carReviewList(CarQBean qBean,HttpServletRequest request, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
...
//		queryMap.put("bpmStates", new String[]{Constants.PROCESS_STATE_REVIEW});//查询流程签约
		...
		log.info(thisMethodName+":end");
		return "car/review/carReviewList";
	}
	...
	public String carReviewUpdate(String id, HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		CarApp bean = null;
		//如果存在
		if (null != id && !"".equals(id)) {
			...
			if(carDecisionList.size() > 0){
				//产品费率
				Product product=productService.queryByName(carDecisionList.get(0).getProduct());
				map.put("product", product);
				//借款金额
				double jkje = 0.0;
				jkje = contract.getLoanAmount();
				//MathUtils.div(carDecisionList.get(0).getAmount(),MathUtils.sub(1,MathUtils.div(product.getSreviceFeeRate(), 100.0)));
...
//				double MAmount = carSignService.getYhkje(jkje, product);
...
//						);
		...
		log.info(thisMethodName+":end");
		return "car/review/carReviewUpdate";
	}
	...
	public void carReviewSave(ReviewFBean reviewFBean, BindingResult bingding,HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception  {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		...
		try {
			if("submit".equals(reviewFBean.getButtonType())){
				...
				if("0".equals(userState)){//不在职 随机分配原信用风险专员所在部门的信用风险专员
					...
					List<CarApp> appList  = this.carAppService.queryList(fmap);
			...
			carReviewService.reviewBpm(state,reviewFBean.getId(), nextState,loginId, processer,logContent1, reviewFBean.getBackMsg(), null);
	...
	public String carReviewRead(String id, HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		CarApp bean = null;
		//如果存在
		...
		log.info(thisMethodName+":end");
		return "car/review/carReviewRead";
	}
}
