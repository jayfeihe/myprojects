...
import com.tera.car.dao.CarDecisionDao;
 ...
 * <b>版权所有：<b>天瑞兴隆<br>
 */
@Service("carDecisionService")
public class CarDecisionService {

	private final static Logger log = Logger.getLogger(CarDecisionService.class);
	...
	@Autowired(required=false) //自动注入
	CarCreditReportService carCreditReportService;
	@Autowired(required=false) //自动注入
	CarAntifraudService carAntifraudService;
	@Autowired(required=false) //自动注入
	CarSummaryService carSummaryService;
	@Autowired(required=false) //自动注入
	CarHousingService carHousingService;
	@Autowired(required=false) //自动注入
	CarInterviewService carInterviewService;
	@Autowired(required=false) //自动注入
	CarCallLogService carCallLogService;
	@Autowired(required=false) //自动注入
	...
	@Autowired(required=false) //自动注入
	CarWageFlowService carWageFlowService;
	@Autowired(required=false) //自动注入
	...
	@Transactional
	public void add(CarDecision... objs)  throws Exception {
		if(objs == null || objs.length < 1){
			return;
		}
		for(CarDecision obj : objs ){
			dao.add(obj);
		}
	}
	
	@Transactional
	public void update(CarDecision obj)  throws Exception {
		dao.update(obj);
	}
	
	@Transactional
	public void updateOnlyChanged(CarDecision obj)  throws Exception {
	...
	public List<CarDecision> queryList(Map<String, Object> map) throws Exception {
		return dao.queryList(map);
	}

	public CarDecision queryByKey(Object id) throws Exception {
		return (CarDecision)dao.queryByKey(id);
	}
	...
	public void verifyUpdate(VerifyFBean verifyFBean,String loginId,String orgId)  throws Exception {
		...
		CarApp app=carAppService.queryByKey(verifyFBean.getId());
		Timestamp newTime=new Timestamp(System.currentTimeMillis());
		
		////保存开始////
		List<CarExt> extList=new ArrayList<CarExt>();
		...
		//信息核查
		for (CarExt carExt : extList) {
			if(carExt.getId()==0 && "1".equals(carExt.getState())){
				carExt.setOperator(loginId);
				carExt.setOrgId(orgId);
				carExt.setState("1");
				carExt.setCreateTime(newTime);
				carExt.setUpdateTime(newTime);
				carExtService.add(carExt);
			}else if(carExt.getId()!=0){
				carExt.setUpdateTime(newTime);
				carExt.setOperator(loginId);
				carExtService.updateOnlyChanged(carExt);
			}
		}
		//影像摘要
		CarSummary summary=verifyFBean.getSummary();
		if(null != summary){
			if(summary.getId()==0 && "1".equals(summary.getState())){
				...
				carSummaryService.add(summary);
			}else if(summary.getId()!=0){
				summary.setUpdateTime(newTime);
				summary.setOperator(loginId);
				carSummaryService.updateOnlyChanged(summary);
			}			
		}
		...
		if(null != wageFlowList){
			for (WageFlow wageFlow : wageFlowList) {
				if(wageFlow.getId()==0 && "1".equals(wageFlow.getState())){
					...
					carWageFlowService.add(wageFlow);
				}else if(wageFlow.getId()!=0){
					wageFlow.setUpdateTime(newTime);
					wageFlow.setOperator(loginId);
					carWageFlowService.updateOnlyChanged(wageFlow);
				}
			}
		}
		//信用卡
		...
		if(null != creditReport){			
			if(creditReport.getId()==0&& "1".equals(creditReport.getState())){
				...
				carCreditReportService.add(creditReport);
			}else if(creditReport.getId()!=0){
				creditReport.setUpdateTime(newTime);
				creditReport.setOperator(loginId);
				carCreditReportService.updateOnlyChanged(creditReport);
			}
		}
		//贷款
		...
		if(null != loanReport){
			if(loanReport.getId()==0 && "1".equals(loanReport.getState())){
				...
				carCreditReportService.add(loanReport);
			}else if(loanReport.getId()!=0){
				loanReport.setUpdateTime(newTime);
				loanReport.setOperator(loginId);
				carCreditReportService.updateOnlyChanged(loanReport);
			}
		}
		// 房产信息
		List<CarHousing> housingList=verifyFBean.getHousingList();
		if(null != housingList){
			for (CarHousing carHousing : housingList) {
				if(carHousing.getId()==0 && "1".equals(carHousing.getState())){
					carHousing.setOperator(loginId);
					carHousing.setOrgId(orgId);
					carHousing.setCreateTime(newTime);
					carHousing.setUpdateTime(newTime);
					carHousingService.add(carHousing);
				}else if(carHousing.getId()!=0){
					carHousing.setUpdateTime(newTime);
					carHousing.setOperator(loginId);
					carHousingService.updateOnlyChanged(carHousing);
				}
			}
		}
		// 信用报告
		...
		// 审核决策
		CarDecision decision=verifyFBean.getDecision();
		if(null != decision){			
			if(decision.getId()==0 && "1".equals(decision.getState())){
				decision.setType(CarDecision.TYPE_VERIFY);
		...
		//面审信息
		List<CarInterview> interviewList=new ArrayList<CarInterview>();
		...
		for (CarInterview carInterview : interviewList) {
			if(carInterview.getId()==0 && "1".equals(carInterview.getState())){
				carInterview.setOperator(loginId);
				carInterview.setOrgId(orgId);
				carInterview.setCreateTime(newTime);
				carInterview.setUpdateTime(newTime);
				carInterviewService.add(carInterview);
			}else if(carInterview.getId()!=0){
				carInterview.setUpdateTime(newTime);
				carInterview.setOperator(loginId);
				carInterviewService.updateOnlyChanged(carInterview);
			}
			List<CarCallLog> callLogList = carInterview.getCallLogList();
			for (CarCallLog carCallLog : callLogList) {
				if(0 == carCallLog.getId() && "1".equals(carCallLog.getState())){
					carCallLog.setOperator(loginId);
					carCallLog.setOrgId(orgId);
					carCallLog.setCreateTime(newTime);
					carCallLog.setUpdateTime(newTime);
					carCallLog.setInterviewingId(carInterview.getId());
					carCallLog.setAppId(carInterview.getAppId());
					carCallLogService.add(carCallLog);
				}else if(0 != carCallLog.getId()){
					carCallLog.setUpdateTime(newTime);
					carCallLog.setOperator(loginId);
					carCallLogService.updateOnlyChanged(carCallLog);
				}
			}
		}
...
//			this.decision(appId, nextState, loginId, processer, logContent1, logContent2, logContent3, null);
		...
		carAppService.updateOnlyChanged(app);		//更新  申请的 状态 与操作员
	}
	...
	public void approvalUpdate(ApprovalFBean approvalFBean,String loginId,String orgId)  throws Exception {
		...
		CarApp app = carAppService.queryByKey(approvalFBean.getId());
		Timestamp newTime=new Timestamp(System.currentTimeMillis());	
		if(decision.getId()==0 && "1".equals(decision.getState())){
			decision.setType(CarDecision.TYPE_APPROVAL);
			decision.setOperator(loginId);
			decision.setOrgId(orgId);
			decision.setCreateTime(newTime);
			decision.setUpdateTime(newTime);
		...
		if("submit".equals(approvalFBean.getButtonType())){
			//提交
				...
				if("00".equals(decision.getDecision())){//不通过到审批决策
					...
					carVerifyOperator = bpmLogList.get(0).getOperator();
					String state = userService.getUser(carVerifyOperator).getState();
					...
					if("0".equals(state)){//不在职 随机分配原信用风险专员所在部门的信用风险专员（挂起不变）
						...
						if (users.size() > 0) {
							User user = users.get(new Random().nextInt(users.size()));
							carVerifyOperator = user.getLoginId();
							app.setCustomerManager(carVerifyOperator);
						}
					}
					//在职 提交到原风险专员签约
					...
					decisionType = "特殊审批";
					carVerifyOperator = approvalFBean.getHigtManagerPeople();
				...
				//流程跳转
				this.decision(decision.getAppId(), nextState, loginId, carVerifyOperator, decisionType, noPassCause, noPassCode, decisionCode, decisionRemark, null, null, null, null, null);
	...
	public void specialApprovalUpdate(SpecialApprovalFBean specialApprovalFBean,String loginId,String orgId)  throws Exception {
		...
		CarApp app = carAppService.queryByKey(specialApprovalFBean.getId());
		Timestamp newTime=new Timestamp(System.currentTimeMillis());
		if(decision.getId()==0 && "1".equals(decision.getState())){
			decision.setType(CarDecision.TYPE_SPACIAL_APPROVAL);
			decision.setOperator(loginId);
			decision.setOrgId(orgId);
			decision.setCreateTime(newTime);
			decision.setUpdateTime(newTime);
		...
		//提交
			String nextState=null;
			String carApprovalOperator = "";//审批决策人
			String decisionType = null;//决策
			String noPassCause = null;//拒绝原因
			...
			if("00".equals(decision.getDecision())){//不通过到审批决策
				...
				carApprovalOperator = bpmLogList.get(0).getOperator();
				decisionType = "退回";
				noPassCause = decision.getReturnMsg(); //退回原因
				app.setState("7");//状态  审批退回
				String state = userService.getUser(carApprovalOperator).getState();
				...
				if("0".equals(state)){//不在职 随机分配原信用风险专员所在部门的信用风险专员（挂起不变）
					...
					if (users.size() > 0) {
						User user = users.get(new Random().nextInt(users.size()));
						carApprovalOperator = user.getLoginId();
						app.setCustomerManager(carApprovalOperator);
					}
				}
				//在职 提交到原风险专员签约
			}else if("05".equals(decision.getDecision())){//拒贷到拒件
			...
			//流程跳转
			this.decision(decision.getAppId(), nextState, loginId, carApprovalOperator, decisionType, noPassCause, noPassCode, decisionCode, decisionRemark, null, null, null, null, null);
			if("05".equals(decision.getDecision()))
				this.decision(decision.getAppId(), Constants.PROCESS_END_APP, loginId, BpmConstants.SYSTEM_SYS, null, null, null, null, null, null, null, null, null, null);
			app.setOperator(loginId);
			app.setUpdateTime(newTime);
	 ...
	 */
	@Transactional
	public void saveEndSpecialAndLoan2match(CarDecision decision,String loginId,String orgId, Timestamp newTime, CarApp app) throws Exception{
		//最终决策  
		decision.setOperator(loginId);
		decision.setOrgId(orgId);
		decision.setCreateTime(newTime);
		decision.setUpdateTime(newTime);
		decision.setId(0);
		decision.setType(CarDecision.TYPE_END);
		dao.add(decision);
...
//		loan2Match.setUseage();
	 ...
	 */
	@Transactional
	public void antifraudUpdate(CarAntifraud carAntifraud,String loginId,String orgId)  throws Exception {
		String appId=carAntifraud.getAppId();
		Timestamp time=new Timestamp(System.currentTimeMillis());
		// 保存
		carAntifraud.setAppId(appId);
		carAntifraud.setSubmitOperator(loginId);
		carAntifraud.setOrgId(orgId);
		carAntifraud.setCreateTime(time);
		carAntifraud.setSubmitInfo(carAntifraud.getSubmitInfo());
		carAntifraud.setState("1");
		carAntifraud.setCreateTime(time);
		this.carAntifraudService.add(carAntifraud);
		
		Map<String, Object> fmap=new HashMap<String, Object>();
		fmap.put("appId", carAntifraud.getAppId());
		List<CarApp> carAppList = carAppService.queryList(fmap);
		CarApp app=carAppList.get(0);
		
		Timestamp newTime=new Timestamp(System.currentTimeMillis());
		//反欺诈
		if("antifraud".equals(carAntifraud.getButtonType())){
			String nextState=Constants.PROCESS_STATE_ANTIFRAUD;
			String submitInfo=carAntifraud.getSubmitInfo();
			
			// 生成随机欺诈处理人
			String processer = this.carAppHandlerService.generateRandProcesser(Constants.ROLE_QZZY, orgId);
...
//			}
		...
		// 解除欺诈
		if("relieveAntifraud".equals(carAntifraud.getButtonType())){
			String nextState = null;
			String result = carAntifraud.getResult();
			String resultInfo = carAntifraud.getResultInfo();
			// 核价节点
...
//			}
			...
			// 获取上一节点处理人
			String processer = this.carAppHandlerService.getPrevProcesser(appId);
		...
		app.setOperator(loginId);
		app.setUpdateTime(newTime);
		carAppService.updateOnlyChanged(app);
	}
	 ...
	 */
	@Transactional
	public boolean saleRefuse(CarApp carApp, String loginId) throws Exception {
		//取到app
		Map<String, Object> appMap = new HashMap<String, Object>();
		appMap.put("appId", carApp.getAppId());
		List<CarApp> appList = carAppService.queryList(appMap);
		CarApp app = appList.get(0);
		Timestamp newTime=new Timestamp(System.currentTimeMillis());
		//更新app表
		app.setSaleRefuseDescribe(carApp.getSaleRefuseDescribe());
		app.setSaleRefuseCode1(carApp.getSaleRefuseCode1());
		app.setSaleRefuseCode2(carApp.getSaleRefuseCode2());
		app.setOperator(loginId);
		app.setUpdateTime(newTime);
		...
		decisionType = "客户放弃";
		noPassCause = carApp.getSaleRefuseDescribe(); //反馈销售描述
		noPassCode = "拒贷码";
		decisionCode = "拒贷码1：" + carApp.getSaleRefuseCode1() + "——拒贷码2：" + carApp.getSaleRefuseCode2();
		//如果有合同
		Map<String, Object> cQuerymap = new HashMap<String, Object>();
		cQuerymap.put("loanAppId", carApp.getAppId());
		...
		this.decision(carApp.getAppId(), nextState, loginId, saleRefuseOperator, decisionType, noPassCause, noPassCode, decisionCode, null, null, null, null, null, null);
		this.decision(carApp.getAppId(), Constants.PROCESS_END_APP, loginId, BpmConstants.SYSTEM_SYS, null, null, null, null, null, null, null, null, null, null);
		return true;
	}
	 ...
	 */
	@Transactional
	public boolean saleRefuseReplenish(CarApp carApp, String loginId) throws Exception {
		//取到app
		Map<String, Object> appMap = new HashMap<String, Object>();
		appMap.put("appId", carApp.getAppId());
		List<CarApp> appList = carAppService.queryList(appMap);
...
//		Timestamp newTime=new Timestamp(System.currentTimeMillis());
		...
		for(int i=0;i<logList.size();i++){
			if("放弃".equals(logList.get(i).getState())){
				...
				needLog.setLogContent2(carApp.getSaleRefuseDescribe());
	...
	public void priceUpdate(PriceFBean priceFBean, String loginId, String orgId) throws Exception {
		...
		CarApp app = carAppService.queryByKey(priceFBean.getId());
		Timestamp newTime = new Timestamp(System.currentTimeMillis());
		if (decision.getId() == 0 && "1".equals(decision.getState())) {
			decision.setType(CarDecision.TYPE_PRICE);
			decision.setOperator(loginId);
			decision.setOrgId(orgId);
			decision.setCreateTime(newTime);
			decision.setUpdateTime(newTime);
