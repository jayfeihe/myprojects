 ...
 * <b>版权所有：<b>天瑞兴隆<br>
 */
@Service("carLendingService")
public class CarLendingService {

	private final static Logger log = Logger.getLogger(CarLendingService.class);
	 ...
	 * @param bean			   申请
	 * @param contract		   合同
	 * @param carDecision 决策
	 * @param loginId
	 * @param orgId
	 * @throws Exception
	 */
	@Transactional
	public ResultObj confirmLending(CarApp bean,Contract contract,CarDecision carDecision,String loginId,String orgId)throws Exception{
		
	
		//得到产品
		Product product=productService.queryByName(carDecision.getProduct());
		// 风险金 费率  5.6%
		double fxRate=0.056;
		//合同金额
		double htAmount=contract.getLoanAmount();
		//放款金额
		double fkAmount=carDecision.getAmount();
		//风险金
...
//			contract.setChannelType("3");// MD
		...
		carAppService.updateOnlyChanged(bean);	//更新 申请 状态
...
//		fmap.put("type", "0");
	 ...
	 */
	@Transactional
	public void backReview(CarApp bean,String loginId, String msg)throws Exception{
		Timestamp ts = new Timestamp(System.currentTimeMillis()); 
		bean.setState("18");
		bean.setOperator(loginId);
		bean.setUpdateTime(ts);
	...
	public ResultObj jimuBoxLending(String appId,String loginId,String orgId) throws Exception{
    		...
    		List<CarApp> appList  = carAppService.queryList(paMap);
    		//得到申请
    		CarApp carApp=appList.get(0);
    		paMap.put("state", "1");
    		paMap.put("type", "0");
    		...
    		//调用 积木盒子 连接请求
//    		return jmboxService.reqJmBox(carApp, decision, contract, loginId, orgId);
    		return null;
	}
	 ...
	 */
	@Transactional
	public void jMErrorconfirmLending(CarApp bean,Contract contract,CarDecision carDecision,
...
//		fmap.put("type", "0");
    	...
    	//更新合同
    	contract.setChannelType(carDecision.getBelongChannel());
	 ...
	 */
	@Transactional
	public void manualJMLendOk(Contract contract,CarDecision carDecision,Date channelDate,String orgId,String loginId) throws Exception{
		//放款成功，更新 合同
		...
		account.setPlanAmount(carDecision.getAmount());
		account.setActualAmount(carDecision.getAmount());
