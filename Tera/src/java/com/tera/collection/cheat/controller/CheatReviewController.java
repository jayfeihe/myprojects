 ...
 */
@Controller
@RequestMapping("/collection/cheat/review")
public class CheatReviewController {
	...
	public String creditReviewQuery(HttpServletRequest request, ModelMap map)
		...
		return "collection/cheat/collectionReviewQuery";
	}
	...
	public String creditReviewList(CollectionQBean qBean,
		...
		queryMap.put("applyType", "2");// applyType 申请类型 application表  1司法 2欺诈
		...
		if(qBean.getState().equals("0")||qBean.getState().equals("")){//0 待处理
		...
		}else if(qBean.getState().equals("1")){//1 已处理 包含欺诈生效和欺诈审批中
			queryMap.remove("state");
			queryMap.put("processName", CollectionConstant.COLLECTION_PROCESS_NAME);//流程名称
			queryMap.put("operator", loginId);
			queryMap.put("logState", CollectionConstant.COLLECTION_STATE_CHEAT_REVIEW);
		...
		log.info(thisMethodName + ":end");
		return "collection/cheat/collectionReviewList";
	}
	...
	public String creditReviewUpdate(String id, HttpServletRequest request,
		...
		map.put("bean", bean);
		log.info(thisMethodName + ":end");
		return "collection/cheat/collectionReviewUpdate";
	}
...
//				SysConstants.LOGIN_USER);
		try {
			collectionCheatService.reviewSave(Judicial, loginId, sessionOrg);
