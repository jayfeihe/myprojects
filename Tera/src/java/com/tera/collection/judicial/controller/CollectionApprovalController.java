...
/**
 * 
 * 司法审批控制器 <b>功能：</b>司法审批<br>
 * <b>作者：</b>zhukun<br>
 * <b>日期：</b>2015-06-08 11:04:29<br>
 * <b>版权所有：<b>天瑞兴隆<br>
 */
@Controller
@RequestMapping("/collection/judicial/approval")
public class CollectionApprovalController {
	/**
	 * 日志
	 */
	...
	public String creditReviewQuery(HttpServletRequest request, ModelMap map) throws Exception {
		...
		return "collection/judicial/collectionApprovalQuery";
	}
	/**
	 * 司法审批的查询列表
	 * 
	...
	public String creditReviewList(CollectionQBean qBean,HttpServletRequest request, ModelMap map) throws Exception {
		...
		if(qBean.getState().equals("3")||qBean.getState().equals("")){//3 待处理 司法审批中
			...
			List<CollectionJudicial> creditAppList = this.collectionAssignService.queryList(queryMap);
		...
		if(qBean.getState().equals("4")){//4 已处理
			...
			queryMap.put("logState", CollectionConstant.COLLECTION_STATE_JUDICIAL_EXAMINE);
			int rowsCount = collectionApplicationService.queryDoneCount(queryMap);
			pm.init(request, rowsCount, null, qBean);
			queryMap.put("rowS", pm.getRowS());
			queryMap.put("rowE", pm.getRowE());
			List<CollectionJudicial> creditAppList = collectionApplicationService.queryDoneList(queryMap);
			pm.setData(creditAppList);
		}
		map.put("pm", pm);
		map.put("state", qBean.getState());
		log.info(thisMethodName + ":end");
		return "collection/judicial/collectionApprovalList";
	}
	...
	public String creditReviewUpdate(String id, HttpServletRequest request,HttpServletResponse response, ModelMap map) throws Exception {
		...
		map.put("bean", bean);
		log.info(thisMethodName + ":end");
		return "collection/judicial/collectionApprovalUpdate";
	}
	/**
	 * 司法审批
	 * @param Judicial
	...
	public void creditReviewSave(CollectionJudicial Judicial,HttpServletRequest request,HttpServletResponse response, ModelMap map) throws Exception {
		...
		try {
			collectionApplicationService.creditReviewSave(Judicial, loginId, sessionOrg);
