 ...
 */
@Controller
@RequestMapping("/credit/assign/verify")
public class CreditAssignVerifyController  extends BaseController{

	/**
	 * 日志
	 */
	private final static Logger log = Logger.getLogger(CreditAssignVerifyController.class);
	...
	public String assignVerifyQuery(HttpServletRequest request, ModelMap map) throws Exception {
		...
		return "credit/assign/verify/assignQuery";
	}
	...
	public String assignVerifyList(CreditQBean qBean,HttpServletRequest request, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		...
		map.put("pm", pm);
		log.info(thisMethodName+":end");
		return "credit/assign/verify/assignList";
	}
