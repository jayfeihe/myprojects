...
import com.tera.credit.model.form.ReviewFBean;
 ...
 * 
 * 信用贷款复核控制器
 * <b>功能：</b>CreditReviewController<br>
...
public class CreditReviewController {
	 ...
	 * 日志
	 */
	private final static Logger log = Logger.getLogger(CreditReviewController.class);
	...
	public String creditReviewQuery(HttpServletRequest request, ModelMap map) throws Exception {
		...
		return "credit/review/creditReviewQuery";
	}
	...
	public String creditReviewList(CreditQBean qBean,HttpServletRequest request, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
...
//		queryMap.put("bpmStates", new String[]{Constants.PROCESS_STATE_REVIEW});//查询流程签约
		...
		log.info(thisMethodName+":end");
		return "credit/review/creditReviewList";
	}
	...
	public String creditReviewUpdate(String id, HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception {
		...
		if (null != id && !"".equals(id)) {
			...
			if(creditDecisionList.size() > 0){
				//产品费率
				Product product=productService.queryByName(creditDecisionList.get(0).getProduct());
				map.put("product", product);
				//借款金额
				double jkje = 0.0;
				jkje = contract.getLoanAmount();
				//MathUtils.div(creditDecisionList.get(0).getAmount(),MathUtils.sub(1,MathUtils.div(product.getSreviceFeeRate(), 100.0)));
...
//				double MAmount = creditSignService.getYhkje(jkje, product);
...
//						);
		...
		log.info(thisMethodName+":end");
		return "credit/review/creditReviewUpdate";
	}
	...
	public void creditReviewSave(ReviewFBean reviewFBean, BindingResult bingding,HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception  {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		...
		try {
			if("submit".equals(reviewFBean.getButtonType())){
				nextState=Constants.PROCESS_STATE_LENDING;
				logContent1="复核通过";
				state="19";		//放款
				List<User> users = userService.getUserByOrgAndRoleAndDepart("86",new String[]{Constants.ROLE_FKZY},null);
				if (users.size() > 0) {
				...
				if("0".equals(userState)){//不在职 随机分配原信用风险专员所在部门的信用风险专员
					...
					List<CreditApp> appList  = this.creditAppService.queryList(fmap);
			...
			creditReviewService.reviewBpm(state,reviewFBean.getId(), nextState,loginId, processer,logContent1, reviewFBean.getBackMsg(), null);
	...
	public String creditReviewRead(String id, HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception {
		String thisMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
		log.info(thisMethodName+":start");
		CreditApp bean = null;
		//如果存在
		...
		log.info(thisMethodName+":end");
		return "credit/review/creditReviewRead";
	}
}
