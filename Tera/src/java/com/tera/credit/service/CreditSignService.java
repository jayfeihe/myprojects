 ...
 * <b>版权所有：<b>天瑞兴隆<br>
 */
@Service("creditSignService")
public class CreditSignService {

	private final static Logger log = Logger.getLogger(CreditSignService.class);
	...
	public ResultObj saveContract(SignFBean signFBean, String loginId, String orgId) throws Exception {
		...
		//保存合同信息
		saveContract(creditApp, contract, loginId, orgId);
			...
			if(!"1".equals(contract.getState())){
				...
				if(null != pro){
			    	...
			    	if(listLoan!=null&&listLoan.size()>0){
			    		...
			    		loan2Match.setLoanAppId(creditApp.getAppId());
			    		loan2Match.setType("1"); // 1 新增  2差额
			    		loan2Match.setMatchType("0");
			    		loan2Match.setAppTime(creditApp.getInputTime());
...
//			    		loan2Match.setLoanServiceRate(pro.getServiceRate());
			    		loan2Match.setOrgId(creditApp.getOrgId());
//			    		loan2Match.setUseage();
	 ...
	 */
	@Transactional
	public Contract saveContract(CreditApp creditApp, Contract contract, String loginId, String orgId) throws Exception{
		...
		if(creditDecisionList.size() > 0){
			contract.setLoanProduct(creditDecisionList.get(0).getProduct());
			contract.setChannelType(creditDecisionList.get(0).getBelongChannel());
			contract.setLoanPeriod(creditDecisionList.get(0).getPeriod());
		...
		contract.setLoanName(creditApp.getName());
		contract.setLoanIdType("01");//身份证
		contract.setLoanIdNo(creditApp.getIdNo());
		contract.setContractClass("03");//信用贷
	...
	public String getContractNo(String appId, Contract contract) throws Exception{
		...
		if(creditVerityList.size() > 0){
			...
			if(creditDecision.getProduct().contains("业主贷"))
				s = "Y";
			else if(creditDecision.getProduct().contains("精英贷"))
				s = "J";
			else if(creditDecision.getProduct().contains("工薪贷"))
				s = "G";
		}
	 ...
	 * @return
	 */
	public double getHtJkje(CreditDecision creditDecision, Product product){
		double htjkje = MathUtils.div(creditDecision.getAmount(),MathUtils.sub(1,MathUtils.div(product.getSreviceFeeRate(), 100.0)));
