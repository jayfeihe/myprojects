 ...
 * <b>版权所有：<b>天瑞兴隆<br>
 */
@Service("houseSignService")
public class HouseSignService {

	private final static Logger log = Logger.getLogger(HouseSignService.class);
	...
	public ResultObj saveContract(SignFBean signFBean, String loginId, String orgId) throws Exception {
		...
		//保存合同信息
		saveContract(houseApp, contract, loginId, orgId);
			...
			if(!"1".equals(contract.getState())){
				...
				if(null != pro){
			    	...
			    	if(listLoan!=null&&listLoan.size()>0){
			    		...
			    		loan2Match.setLoanAppId(houseApp.getAppId());
			    		loan2Match.setType("1"); // 1 新增  2差额
			    		loan2Match.setMatchType("0");
			    		loan2Match.setAppTime(houseApp.getInputTime());
...
//			    		loan2Match.setLoanServiceRate(pro.getServiceRate());
			    		loan2Match.setOrgId(houseApp.getOrgId());
//			    		loan2Match.setUseage();
				...
				if(result.isFlag()){
					...
					//   t_house_app   当前申请状态 置为 14（撮合已完成）
	 ...
	 */
	@Transactional
	public Contract saveContract(HouseApp houseApp, Contract contract, String loginId, String orgId) throws Exception{
		...
		if(houseDecisionList.size() > 0){
			contract.setLoanProduct(houseDecisionList.get(0).getProduct());
			contract.setChannelType(houseDecisionList.get(0).getBelongChannel());
			contract.setLoanPeriod(houseDecisionList.get(0).getPeriod());
		...
		contract.setLoanName(houseApp.getName());
		contract.setLoanIdType("01");//身份证
		contract.setLoanIdNo(houseApp.getIdNo());
		contract.setContractClass("01");//房贷
	...
	public String getContractNo(String appId, Contract contract) throws Exception{
		...
		if(houseVerityList.size() > 0){
			...
			if(houseDecision.getProduct().contains("业主贷"))
				s = "Y";
			else if(houseDecision.getProduct().contains("精英贷"))
				s = "J";
			else if(houseDecision.getProduct().contains("工薪贷"))
				s = "G";
		}
	 ...
	 * @return
	 */
	public double getHtJkje(HouseDecision houseDecision, Product product){
		double htjkje = MathUtils.div(houseDecision.getAmount(),MathUtils.sub(1,MathUtils.div(product.getSreviceFeeRate(), 100.0)));
