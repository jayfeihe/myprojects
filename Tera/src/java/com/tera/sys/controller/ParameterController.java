...
public class ParameterController {
	...
	public String parameterList(HttpServletRequest request, ModelMap map) throws Exception {
		...
		int rowsCount = this.parameterService.queryCount(queryMap);
		pm.init(request, rowsCount, null, bean);
		queryMap.put("rowS", pm.getRowS());
		queryMap.put("rowE", pm.getRowE());
		List<Parameter> parameterList = this.parameterService.queryList(queryMap);
	...
	public void parameterSave(HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception  {
		...
		try {
			...
			//如果存在
			if (bean.getId() != 0) {
				this.parameterService.updateOnlyChanged(bean);
				...
				if (!oldParameter.getParamValue().equals(bean.getParamValue())) {
					//修改了催收升降期参数，做相应处理
					if (CollectionConstant.COLLECTION_AGE_LIMIT.equals(bean.getParamName())) {
						this.parameterService.updateOnlyChanged(bean);
						collectionBatchService.changeAgeLimit();
					}
				}*/
			} else { //如果不存在
