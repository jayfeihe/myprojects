...
import com.tera.sys.model.form.RoleQBean;
...
//		map.put("roleTypes", roleTypes);
	...
	public String sysRoleList(HttpServletRequest request, ModelMap map) throws Exception {
		...
		RoleQBean roleQBean = (RoleQBean) RequestUtils.getRequestBean(RoleQBean.class, request);
		Map<String, Object> beanMap = null;
		beanMap = ObjectUtils.describe(roleQBean);
		int rowsCount = roleService.getRoleCount(beanMap);
		pm.init(request, rowsCount, null, roleQBean);
		beanMap.put("rowS", pm.getRowS());
		beanMap.put("rowE", pm.getRowE());
		List<Role> roles = this.roleService.queryRole(beanMap);
		pm.setData(roles);
		map.put("pm", pm);
		log.info(thisMethodName+":end");
