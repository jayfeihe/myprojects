...
<%-- <section id="advert-gallery-wrapper">
	<c:forEach items="${pager.datas }" var="data">
		<div class="mask white-panel single-box" >
			<div class="waterfall" style="position: relative;">
			<dl>
				<dt>
					<a href="<%=basePath%>advert/detail?id=${data.id }" class="liua" target="_blank">
...
<div id="advertPage"> 
	<jsp:include page="/WEB-INF/jsp/page.jsp">
		<jsp:param value="Advert" name="type"/>
	</jsp:include>
</div> --%>
<input type="hidden" id="totalCount" value="${pager.total}"/>
<input type="hidden" id="curAdvertCount" value="${fn:length(pager.datas)}"/>
<div id="advertFall" class="wookmark">
 	<c:forEach items="${pager.datas }" var="data">
	    <div class="mask">
	       	<div class="waterfall">
	       		<div style="height: ${data.thumbHeight}px" class="image-box">
		       		<a href="<%=basePath%>advert/detail?id=${data.id }" target="_blank">
		       			<img class="waterImg" src="<%=basePath%>dataRecord/imgRead?imgType=2&adpicUrl=${data.adpicUrl }">
		       		</a>
		       	</div>
...
<script type="text/javascript">
	waterfall.init({
		imageParent:'advertFall',
		container:'confirmAdvertIllegal',
		imageCount:$("#curAdvertCount").val()
	});
</script>
