...
<!-- 	<link href="http://cdn.bootcss.com/font-awesome/4.6.3/css/font-awesome.min.css" rel="stylesheet"> -->
	...
	<style type="text/css">
		#advert-gallery-wrapper,#creative-gallery-wrapper  {
			position: relative;
			/* max-width: 75%;
			width: 75%;  */
			margin: 20px auto;
		}
		#advertPage,#creativePage {
			background: #fff;
...
<body>
			...
			<div class="col-md-12">
				<form class="form-inline" style=" text-align: center;" role="form">
					<div class="form-group" style="margin-right:12px;">
						...
						<input class="form-control big-search"  id="keyWord" value="${keyWord }"
							placeholder="<c:if test='${login_company.type eq 1 }'>可输入广告名称/创意名称/媒体名称查询</c:if><c:if test='${login_company.type eq 2 }'>可输入广告名称/创意名称/广告主名称查询</c:if>">
					</div>
					<input type="button" class="btn btn-yellow btn-ww" value="GO" onclick="queryAdvert();"/>
				</form>
			</div>
		</div>
			...
			<div class="col-md-12">	
				<ul class="nav nav-tabs fucb" role="tablist" style="padding: 0px 40px; text-align: center;">
					<li role="presentation" class="active">
						<a href="#home" class="fuca" role="tab" data-toggle="tab" id="advert">
							<span class="title">违法广告活动</span>
							<span class="content">
								共计<span class="impor changeColor" id="advertCount"></span>个
							</span>
						</a>
					</li>
					<li role="presentation">
						<a href="#profile" class="fuca" role="tab" data-toggle="tab" id="creative">
							<span class="title">违法广告创意</span>
							<span class="content">
								共计<span class="impor" id="creativeCount"></span>个
							</span>
						</a>
					</li>
					<li role="presentation">
						<a href="#messages" class="fuca" role="tab" data-toggle="tab" id="related">
							<span class="title">
								<c:choose>
									<c:when test="${login_company.type eq 1 }">投放媒体</c:when>
									<c:when test="${login_company.type eq 2 }">涉及广告主</c:when>
								</c:choose>
							</span>
							<span class="content">
								共计<span class="impor" id="relatedCount"></span>个
							</span>
						</a>
					</li>
				</ul>
				...
				<div class="tab-content illegal2">
					<div role="tabpanel" class="tab-pane active" id="home">
						<div class="row">
							<div class="col-md-12">
								<form class="form-inline search" role="form" id="confirmAdvertForm">
									<div class="pull-left datePanForm" id="dingLeft">
					        			<img class="icon" src="<%=basePath%>images/rili.png">
						        		<input type="text" class="form-control dateInput" id="dateChange" name="dateChange"
							        	...
							        	<div class="datePan" id="datePan">
					                        ...
					                        <div>
						                        <input type="button" class="btn btn-ssp" value="确定" onclick="bindDate('confirmAdvertForm');"/>
						                     	<input type="button" class="btn btn-ssp" value="取消" onclick="cancelDate('confirmAdvertForm');"/>
					                        </div>
			                    		</div>
			        				</div>
									...
									<div class="form-group">
										...
										<select class="form-control" name="level">
											<option value="">违法程度</option>
											<option value="<%=DataRecord.LEVEL_SEVERE %>">严重违法</option>
											<option value="<%=DataRecord.LEVEL_COMMON %>">一般违法</option>
											<option value="<%=DataRecord.LEVEL_LIGHT %>">轻微违法</option>
										</select>
									</div>
									...
									<input type="button" class="btn btn-yellow" value="搜索" onclick="loadConfirmAdvertIllegal(1)"/>
								</form>
								<div id="confirmAdvertIllegal"></div>
							</div>
						</div>
					</div>
					...
					<div role="tabpanel" class="tab-pane" id="profile">
						<div class="row">
							<div class="col-md-12">
								<form class="form-inline search" role="form" id="confirmCreativeForm">
									<div class="pull-left datePanForm">
							        	...
							        	<div class="datePan" id="datePan">
					                        ...
					                        <div>
						                        <input type="button" class="btn btn-ssp" value="确定" onclick="bindDate('confirmCreativeForm');"/>
						                     	<input type="button" class="btn btn-ssp" value="取消" onclick="cancelDate('confirmCreativeForm');"/>
					                        </div>
					                    </div>
					        		</div>
									...
									<input type="button" class="btn btn-yellow" value="搜索" onclick="loadConfirmCreativeIllegal(1)"/>
								</form>
								<div id="confirmCreativeIllegal"></div>
							</div>
						</div>
					</div>
					
					<div role="tabpanel" class="tab-pane" id="messages">
						<div id="relatedArea" class="addisplay related-list">
				...
				<!-- ***************************************对应问号的定位内容开始************************************ -->
				<div class="wen_top" style="position:absolute;background:#ccc;width:250px;height:60px;left:354px;top:70px;padding:5px 10px;display:none;z-index:9999999999999;border-radius:5px;">
    ...
    <script type="text/javascript" src="<%=basePath %>js/confirm/confirm.js"></script>
    <script type="text/javascript" src="<%=basePath %>js/feedback.js"></script>
    
    <script type="text/javascript">
    	$(function(){
    		feedback.openModel('confirmAdvertIllegal');
    		feedback.openModel('confirmCreativeIllegal');
    		feedback.closeModel();
    		feedback.init('correctConfirmForm');
    		feedback.init('correctedForm');
    		feedback.submitCorrect({'pageType':'1','page':'1'});
