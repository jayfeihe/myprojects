...
public class GenerateKeyUtil {
	...
	static {
		...
		/**广告主、媒体主键id前缀*/
		SeqCreateVo UCVo = new SeqCreateVo(0, DateUtil.formatDate(new Date(), DateUtil.FORMAT_DD_NO_SYMBOL), CommonConstants.PREFIX_USER_COMPANY);
		idHashtable.put(CommonConstants.PREFIX_USER_COMPANY, UCVo);
		
		/**用户账户主键id*/
		SeqCreateVo UAVo = new SeqCreateVo(0, DateUtil.formatDate(new Date(), DateUtil.FORMAT_DD_NO_SYMBOL), CommonConstants.PREFIX_USER_ACCOUNT);
		idHashtable.put(CommonConstants.PREFIX_USER_ACCOUNT, UAVo);
		
		/**用户登录日志主键前缀*/
		SeqCreateVo ULVo = new SeqCreateVo(0, DateUtil.formatDate(new Date(), DateUtil.FORMAT_DD_NO_SYMBOL), CommonConstants.PREFIX_USER_LOGIN);
		idHashtable.put(CommonConstants.PREFIX_USER_LOGIN, ULVo);
	}
