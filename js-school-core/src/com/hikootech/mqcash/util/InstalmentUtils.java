...
//		System.out.println(remainder);
	...
	public static BigDecimal calOverdue(BigDecimal orderAmount, BigDecimal overdueRate, Date repayPlanTime){
		Date curTime = DateUtil.dayOfOrigin(new Date());
	...
	*/
	...
	public static BigDecimal calcPayAmount(List<UserRepaymentPlans> plansList){
		BigDecimal retVal=new BigDecimal("0.00");
		for (UserRepaymentPlans plan : plansList) {
			if(plan==null){
				continue;
			}
			retVal=retVal.add(calcPayAmount(plan));
		}
		return retVal;
	}
	...
	/**根据传入的一个还款计划，查询出最终需还款的金额***/
	public static BigDecimal calcPayAmount(UserRepaymentPlans plan){
		...
		BigDecimal receivableOverdue =plan.getReceivableOverdue()==null?new BigDecimal(0):plan.getReceivableOverdue();
		...
		BigDecimal receivedOverdue=plan.getReceivedOverdue()==null?new BigDecimal(0):plan.getReceivedOverdue();
		BigDecimal reduceOverdue=plan.getReduceOverdue()==null?new BigDecimal(0):plan.getReduceOverdue();
	...
	/** 根据传入的一个还款计划，查询出最终需还款的金额 ***/
	public static BigDecimal calcPayAmount(UserRepaymentPlansDTO plan) {
		BigDecimal retValTemp = new BigDecimal("0.00");
		if (plan == null) {
			return retValTemp;
		}
		...
		retValTemp = retValTemp.add(receivablePrincipal).add(receivableService)
				.add(receivableOverdue).subtract(receivedPrincipal)
				.subtract(receivedService).subtract(receivedOverdue)
				.subtract(reduceOverdue);
